...
| 7K | リーダー攻撃先行 (Step 0.8) + blocker 遅延 (Step 1) | [Section 1](#1-greedyai-の行動優先順位--各-1-手の判断フロー) |
| 7L | (deckbuilder のみ、 AI 動作変更なし) | deckbuilder.py |
| 7M | (deckbuilder のみ、 archetype-aware) | deckbuilder.py |
...
### ⭐ Step 0.8: リーダー攻撃先行 (Phase 7K、 NEW)

- リーダーが active + opp.leader を単独で超え (or 1 DON で届く) なら **即攻撃**
- 戦略原理: ノーリスク機会 (= 手札を消費せず -1 ライフ / counter 要求)、 不確実性最大化
- desperate bluff モード時 (Section 12) は skip
- 起動メイン後回しケース (Step 0) も skip
...
2. **⭐ ブロッカー play 遅延** (Phase 7K extend、 NEW):
   - 攻撃可能なターンでは ブロッカーカードを 後回し (= 攻撃後に play)
   - 戦略原理: opp の life trigger が KO 系の場合、 先に play した blocker を失う
   - 攻撃で trigger 処理 → blocker play で安全に着地
3. **`early_finisher_hold` フィルタ** (= 高コストフィニッシャー温存):
   - 自ライフ ≥ `finisher_hold_life` (デフォルト 3) なら finisher を **手札に温存**
   - 中盤に finisher を切ってしまうと終盤の押し込み力が落ちる
4. **`synergy_feature_priority`** (= 特徴シナジー):
   - 該当特徴を持つカードがあれば、 そこから選ぶ
5. **ソート順** (= 同じ特徴フィルタ後の選び方):
...
`_compute_lethal_action` で以下を計算 (キャラ KO より優先)。 Phase 7B/J で再設計済:

1. **⭐ 配分プランナー (Phase 7J)** `plan_optimal_attack_sequence` で最適 DON 配分を計算:
   - 各 attacker に opp.leader 超過分の最低 DON を振る
   - 余り DON は最弱 attacker から +1 ずつで **均等化** (= ±2k マージン)
   - 弱→強順で配置 (= counter 吸わせから本命)
   - 戦略原理 (OPTCG コミュニティ): 1 大型より複数均等の方が要求値高い
2. **plan.is_lethal && len(plan.sequence) >= hits_needed** が False なら諦め
3. **⭐ 確率ベース判定 (Phase 7B + 7G + 7H、 NEW)**:
   - `expected_counter_from_don_bluff` で opp の visible DON から counter event 期待寄与を加算
   - `probability_counter_total_at_least` でハイパージオメトリック分布から P(opp が止める) を計算
   - **risk-adjusted threshold**: fallback_win_prob (= 1 - opp_next_lethal) で動的:
     - ≥ 0.7: 0.75 (= 慎重)
     - 0.4-0.7: 0.70 (= 標準)
     - 0.2-0.4: 0.55 (= 不利、 50/50 でも行く)
     - < 0.2: 0.40 (= 負け濃厚、 ブラフちぎって賭け)
4. **`p_lethal >= threshold`** ならリーサル成立、 最初のアクション返却:
   - DON 付与が必要な attacker があれば、 まず 1 枚付与
   - 全部届くなら 最大 excess 攻撃から (= 確実に通す)

**OPTCG 文脈 (= Phase 7 後)**:
- 旧 1.2x マージン → 確率分布 + risk-adjusted threshold (= 状況依存)
- 「アグロ vs アグロ で 不利なら 40% リーサル attempt」 = ブラフ ちぎって賭け
- 「コントロール vs アグロ で 75% 必要」 = 慎重判定で安全策
- ダブルアタックは無視 (= simplification、 未対応)
- バニッシュは無視 (= life→hand を計算してない、 未対応)

**懸念点 (= 残課題)**:
- ダブルアタック / バニッシュ の打点 2 計算 未実装
- ライフトリガー の対策見越し (= 雷迎 等) は Step 2c で限定的に対応
...
## 4. 防御判断 (`choose_defense`) — Phase 7A で 3-tier 化

相手が攻撃してきた時、 「ブロッカー切るか」 「counter 何枚切るか」 を決める。

### ⭐ ステップ 1: 3-tier ブロッカー選択 (Phase 7A、 NEW)

旧実装の単一判定 (= 「自力生存 or 特攻」) を 3-tier に拡張:

**Tier 1 (block_safe)**: 自力で生存
- `c.power > atk_p` (= **strictly greater**、 公式 7-1-4 準拠、 旧 `>=` バグ修正済)
- 最も power 高い blocker を採用

**Tier 2 (block_rescue)** ⭐ NEW (Phase 7A):
- 自力では負けるが、 counter で救える blocker
- 価値判定: `_is_valuable_blocker` で 5000+ power / finisher・removal・blocker role / cost ≥ 4
- 必要 counter コスト判定: `_is_rescue_worthwhile` で
  - power ≥ 5000: 2 枚 / 2000 counter まで
  - power ≥ 3000: 1 枚 / 1000 counter まで
  - life ≤ 2 で finisher/blocker role: 2 枚 / 3000 まで
- 最安 rescue を選択 → forced_counter で確定

**Tier 3 (block_sacrifice)**: life ≤ 1 の特攻
- 自力 NG + rescue NG でも 最後の防衛として blocker 投入
...
**懸念点 (= Phase 7A 後 残課題)**:
- ✅ ブロッカー判定の `>=` バグ → Phase 7A で `>` に修正済
- ✅ valuable blocker の counter rescue 戦略 → Phase 7A で実装
...
## 6. ヒドゥンインフォメーション処理 (= 相手の手札推定) — Phase 7B/E/I で 大幅強化

`engine/hand_estimator.py` で「相手の手札は見えない」 という前提のモデル化。

### Phase 7B/E/I 後の構造

- **公開情報**: opp.hand の枚数、 opp.deck + opp.hand のカード集合、 trash, play, life の公開済カード、
  ⭐ `opp.known_hand_card_ids` (Phase 7I): 「公開経路で hand に入ったカードの ID」
- **隠匿情報**: 未知カードのうち どの N 枚が hand に分配されたか

### 主要 API (Phase 7B/I 後)

| 関数 | 何を返すか | Phase |
|---|---|---|
| `expected_counter_per_card` | プール平均カウンター値 (= 旧 API) | 既存 |
| `expected_counter_total` | 上記 × opp.hand_count (= 旧) | 既存 |
| ⭐ `counter_total_pmf` | counter 合計の **確率分布** (= ハイパージオメトリック) | **7B** |
| ⭐ `probability_counter_total_at_least(state, opp_idx, threshold)` | 「合計 ≥ threshold」 の確率 | **7B** |
| ⭐ `counter_total_quantile(state, opp_idx, q)` | q-分位点 (= 0.5 中央値, 0.9 上限) | **7B** |
| ⭐ `expected_counter_from_don_bluff` | opp の active DON から counter event 寄与推定 + archetype factor | **7G/H** |
| `probability_of_blocker_in_hand` | ≥1 ブロッカー の確率 | 既存 |

### Phase 7I: 公開済カード追跡

- `return_to_hand` / `search` 等の経路で hand に入ったカードは `known_hand_card_ids` に追加
- pmf 計算で **「既知 + 未知」 分離**:
  - known portion = 確定 counter 値 (= 加算)
  - unknown portion = (deck + hand - 既知) からハイパージオメトリック
  - total pmf = known_counter で shift した unknown pmf
- 全 known で counter event 無し → bluff 完全に 0

### Phase 7E: pool をメタデッキ仮定で精密化

- 旧: `_opponent_pool` = `opp.deck + opp.hand` 直読 (= 「ズル」 モード)
- 新: classifier で opp archetype 高信頼推定なら archetype 代表 recipe - 観測済 = pool
- 低信頼時は fallback で旧挙動 (= opp.deck + opp.hand)
- `set_pool_mode(use_classifier=True/False)` で切替可能

### 使われ方

- リーサル判定で `probability_counter_total_at_least` を主軸に
- bluff 判定 (Section 12) で classifier 出力 + visible DON を組合せ
- opp_hand_threat 指標 (= board_eval) は引き続き期待値ベースで近似

**OPTCG 文脈 (= Phase 7 後)**:
- 「相手のトラッシュにカウンター 5 枚行った」 → 自動でプール縮小 → pmf 再計算
- 「return_to_hand で見えた blocker」 → known_hand に追加 → opp の defense 推定が確定的
- 「相手が active DON 2 残してる」 → archetype 別 bluff factor で counter event 期待寄与 加算

**懸念点 (= 残課題)**:
- mulligan による偏り (= 「先攻 1 ターン目 で counter 引かない」 確率) は未考慮
- カード固有の role 別偏り (= 「《空島》軸なら finisher 多く持つ」) は弱い
- life cards の trigger 公開後の追跡 (= life から hand に入ったか trash 行ったか) は限定的
...
## 8. MatchupProfile (= 対戦相手別の動作 override) — Phase 7D で動的化
...
### ⭐ Phase 7D: ターン毎再評価 (NEW)

旧: 初回 `choose_action` で lazy-load、 同一試合中は固定
新: `_last_matchup_eval_turn` で **ターン毎に再評価**

- archetype 推定は **classifier (Phase 7C) 経由** で動的に決定
  (= 旧 static map は fallback)
- 各ターン頭で `_base_defense_thresholds` / `_base_attack_gap_tolerance` /
  `_base_finisher_hold_life` にリセット → MatchupProfile を再評価 → override 再適用
- 観測カードが増えれば classifier 精度向上 → MatchupProfile も refine

`infer_opponent_archetype_with_confidence` で信頼度 (= P(top archetype)) も取得可能。

**OPTCG 文脈 (= Phase 7D 後)**:
- 旧: 「アグロ vs コントロール」 と固定判定 (= leader 単独で archetype 推定)
- 新: ターン経過で 同 leader でも variant 検出 (= 「event 軸 vs chara 軸」 の分離)
- classifier confidence が低い時 (= 序盤 + 観測薄い時) は default fallback で安全

**懸念点 (= 残課題)**:
- profile JSON の中身が手書き / 経験的。 実戦データから自動学習されてはいない
- 16 デッキ × 16 デッキ全マッチアップで個別調整 されてない
- variant 別 (= 「event 軸 紫エネル」 vs 「chara 軸」) の profile 個別化は将来課題
