...
## 公式 Q&A conformance バッチ (2026-08-11、 cron `optcg-faq-conformance`): conform 16 / n/a 2 / escalated 1

エンジン無変更 (全件 conform/n/a/escalated)。 再調査回避のため要点のみ記録。

### conform (公式どおり、 overlay/engine 検査で確認)

- **OP13-012 ネフェルタリ・ビビ** (866a9541eb93): 【登場時】 filter `cost_ge:2, feature_in:[アラバスタ王国,麦わらの一味]` → コスト1の麦わらの一味は加えられない。
- **OP16-098 ヤマト** (886e111d2a59): 【起動メイン】 `play_from_trash` filter `cost_eq:8, color:黒, name:ヤマト` → コスト8ちょうどのみ (7以下/9以上不可)。
- **OP10-026 錦えもん** (87ade4d021cb): 【起動メイン】 cost = [`return_self_to_deck_bottom` (=場のこのキャラ), `trash_to_deck` name錦えもん power0 limit1] → トラッシュの錦えもん2枚は不可 (場のこの1枚+トラッシュ0power1枚)。
- **OP14-009 トラファルガー・ロー** (8800edc44d0e): 【相手のアタック時】 `swap_base_power_self_leader_chara` = 自リーダー+自キャラ限定 → 相手キャラは選べない。
- **OP10-067 セニョール・ピンク** (89b985106282): 【登場時】 do=[`search_from_trash`(0枚可), `untap_don`1] は独立 → イベントを加えなくてもドン1枚アクティブ可。
- **OP10-057 レオ** (88ecc9a2f782): 【登場時】 conditional(leader=ウソップ) 内に `search_top_n` と `trash_self_hand_random` 両方 → 加えた枚数によらず必ず手札1枚捨てる ([dd-mand])。
- **OP05-062 おナミ** (8924df29813e): `name_matches` は完全一致+別名map(substring非対応) → 名前フィルタ「ナミ」は「おナミ」に不一致 = カード名「ナミ」として扱わない。
- **OP15-119 ルフィ** (895409497f09): reactive `when:opp_event_played` は `trigger_main_event`/`trigger_counter_event` (=イベント発動経路) からのみ発火。ライフ【トリガー】が【メイン】/【カウンター】効果を実行しても発火せず = 「【トリガー】発動 ≠ イベント発動」。
- **EB01-047 ラブーン** (89720580afa1): overlay に `on_self_chara_ko` を保持 → 自身のみKOでも自身の「キャラがKOされた時」が発火 (既存 enqueue 粒度 fix)。
- **OP15-077 雷龍 → OP15-119** (863cb9b763af): `stay_rested_next_refresh` はターゲット解決時 (power6000以下) にフラグを set-once。後で別効果でパワー7000+になっても再評価されず、次リフレッシュで非アクティブ維持。
- **OP15-119 相手【ブロッカー】発動時** (89848c64434a): `engine/game.py` L1777-1778 = `trigger_on_opp_blocker_use` (ターンP=攻撃側 reactive) → `trigger_on_block` (非ターンP ブロック時) の順。公式どおり reactive 解決後にブロック時。
- **OP13-003 ゴール・D・ロジャー** (88d5447262df): `auto_attach_to_leader` は `if self_don_ge:1` で gate。最初のターン (場ドン0) は付与されずコストエリアへ (公式6-5-5)。
- **P-116 ニコ・ロビン** (886553e2f4de): 【KO時】 `if self_trash_count_ge:7`。KO時はキャラがトラッシュに入った後に解決 → トラッシュ6→7で条件成立、ドロー+捨て発動。
- **ST20-002/ST09-010 系 効果KO置換** (8857c26569c7): 「代わりにライフ1枚をトラッシュ」は cost に配置 (2026-08-09 sweep) で payability gate。ライフ0では支払えず置換不成立 = KOされる。「代わりにKOされない」はできない。
- **OP11-059 ゴムゴムの王蛇** (87aabf51af58): 【カウンター】 **実測確認**。手札5枚 (自身含む) から発動 → 自身が手札を離れ手札4枚 → `self_hand_count_le:4` 成立 → +2000追加 = 合計+4000 (リーダーに +4000 を実測)。手札6枚では +2000 のみ。既存 ST03-017 テストと同一機構。
- **ST08-013 Mr.2・ボン・クレー** (89acd443a6bb): **実測確認** (防御側でバトル負けKO)。KO後トラッシュにあるキャラの `on_self_battled` (ドン×1) は発火せず = 相手アタッカーをKOしない。付与ドンもKO時にコストエリアへ戻り `self_attached_don_ge:1` も不成立。

### n/a (engine で表現不能)

- **「すべてのカード名と特徴と属性を持つ」リーダー** (86472de66fe8 / 89bdb05a05c6): 全カードDB走査で該当リーダー0件 (未収録/未実装)。全名リーダーがコトリ/サトリ (OP15-072 ホトリ条件) やツノッコフ (OP16-025 ウサッコフ条件) の名前条件を満たすかの一般則だが、対象リーダーがDBに無く engine で構成できない。

### escalated (対象特定不能)

- **OP11 KO時 vs OP01-026** (89444ee8a4c4): 「自分のターン中に相手が OP01-026 を発動してこのキャラをKOした時、このキャラの効果は発動できますか→いいえ」。OP11のKO時トリガー保有カード (OP11-024/035/051) は「相手の効果でKOされた時」で OP01-026 (相手効果) に発火する = 答え「いいえ」と矛盾。参照カードが一意に定まらず要人間レビュー。

## 2026-08-11 #4 — pending 消化バッチ 2 (台帳末尾から 20 件、 4 fixed / 15 conform / 1 escalated)

### 【fixed】「自分のキャラが特徴《X》を持つキャラ **のみの場合**」 は キャラ0枚では成立しない

- **一次情報** (cardqa_op_16 / OP16-022 ルフィ): 「自分のキャラが0枚の時、 この【起動メイン】効果で
  ドン‼をアクティブにできますか？」 → 「**いいえ、 できません**」。
  (cardqa_eb_03 / EB03-038 ごち♡ も同旨で 「いいえ」)
- **違反**: `self_chara_only_feature` / `self_chara_only_feature_contains` が **空集合を vacuous
  True** にしており (コメントにも 「空でも True」 と明記)、 キャラ0枚でも成立していた。
- **是正**: `if not me.characters: return False` を両条件に追加 (Python / Rust)。
  同型条件を使う **6 枚** (EB02-010 / OP05-084 / OP05-092 / OP16-022 / EB03-038 / OP11-043) に効く。

### 【fixed】「元々のパワーを◯◯にする」 が複数適用 → **最も高い値** (公式 4-9-2-1)

- **一次情報** (総合ルール 4-9-2-1 逐語 + cardqa_st_34 / ST34-004 シャーロット・リンリン
  「相手のキャラ1枚までを、 このターン中、 元々のパワー0にする」):
  「同じキャラに適用されている元々のパワーを変更する効果のうち、 **最も高い値** である元々の
  パワーに適用されます」
- **違反**: engine は **後勝ち (last-wins)** で、 後から掛けた 0 が 6000 を潰していた。
- **是正**: ① 同一 slot への 「元々の」 書き換えは **max** を採る (`_apply_original_base_power`)
  ② `truly_original_power` も **全 slot の最大値** を返す (優先順位で 1 つ選ぶと低い値が高い値を
  潰す)。 Python / Rust 両方。
- ⚠ **素の 「パワーが◯◯になる」 (`original: false`) は 4-9-2-1 の対象外** = 後勝ちのまま。

### 【fixed】OP04-081 キャベンディッシュ / OP04-091 レオ — 「その後 デッキ上2枚をトラッシュ」 が欠落

公式 (cardqa_op_04): 「デッキのカードをトラッシュに置かないことはできますか？」 → 「**いいえ**」。
overlay は KO しか実装しておらず mill が **丸ごと無かった**。 コロン前が発動コストなので
`optional_cost_then` の `effect` 末尾に置く (= 払わなければ mill も起きない)。 掃引で同型は 2 枚のみ。

### conform (15 件、 要点)

- search は **デッキが depth 未満でもあるだけ見る** (デッキ0枚とはみなさない)
- 任意コスト 「自分のキャラ1枚を手札に戻す」 に **自分自身** を選べ、 その後の効果も走る (OP09-030)
- 「リーダーが特徴Xを持つ場合」 は **効果全体** の gate = 「その後」 も含めて不発 (OP14-112 / EB02-028)
- 「ドン‼-N(…できる)：」 は 人間に `optional_cost_confirm` が立ち **支払わない選択ができる**
- コロン前が置換不能なら発動できない = 後半も起きない (OP09-101 クザン)
- `on_self_don_returned_to_deck` は **戻された原因を問わない** (相手の効果でも発火、 ST10-007)
- `hand_or_trash_to_self_life` は **手札とトラッシュを横断** して数える (ST13-003)
- 【ドン‼×N】gate は **コスト支払い後** に評価される (OP10-069 闘魚: 付与ドンが0になれば KO しない)
- `reveal_top_then` の `rest_remain: top` = 公開したカードはデッキの一番上に戻る (ST22-012)
- `duration` 付き一括適用は **発動後に登場したキャラに乗らない** (OP14-082)
- 「ドン‼1枚をレストにし」 は ① と同じ発動コスト (`rest_self_don`、 OP08-082)
- **効果を無効にしても既に適用済のパワー修正は残る**
- 独立した target 解決なので -3000 と KO は別のキャラを選べる (OP04-008)

### escalated (1 件、 既存 83e1770aba3a と同一原因に合流)

ST13-003 ルフィ(L) 「表向きライフは手札に加わる代わりにデッキの下に置かれる」 × ST13-012 マキノ の
発動コスト 「ライフ1枚を手札に加えることができる：」 の相互作用 (公式: デッキの下には置けるが
**コストを支払えていないので何も起きない**)。 **ライフの per-card face-up 追跡** が要る点で
83e1770aba3a と同根 → 同じ canonical state 変更 (life を (card, face_up) 対にする、
...
## 2026-08-11 #6 — pending 消化バッチ 3 (末尾から) (台帳末尾から 20 件、 2 fixed / 18 conform)

### 【fixed】OP15-003 アルビダ — 払えない発動コストで **発動でき、【ターン1回】だけ消費** していた

- **一次情報** (cardqa_op_15): 「相手のキャラが0枚のときや、 相手のレストのドン‼が相手のコスト
  エリアに無い時に、 この【起動メイン】効果で自分のリーダーやキャラに自分のドン‼を付与すること
  はできますか？」 → 「**いいえ、 できません**」
- **違反**: 「相手のキャラ1枚に相手のレストのドン‼1枚を付与できる：」 は **コロン前 = 発動コスト**。
  `_optional_cost_payable_in_do` が `attach_opp_don_to_opp_chara` を見ておらず、 払えなくても
  起動メインが legal に出て、 何も起きないまま **【ターン1回】だけ消費** していた。
- **是正**: 同 helper に payability を追加 (Python / Rust)。 実測: 相手キャラ0 / 相手レストドン0 →
  候補 0、 両方あり → 1。

### 【fixed】OP10-119 ロー / OP12-031 たしぎ — 「その後、 …レストのドン‼N枚を付与する」 が欠落

overlay に前半しか無く、 後半のドン付与が **丸ごと落ちていた**。 nested 構造 (optional_cost_then /
conditional) まで含めて掃引した結果、 同型の未実装は **この 2 枚のみ**。
OP10-119 は 「自分の**特徴《超新星》を持つ**リーダー1枚に」 なので `conditional(leader_feature)` で包む
(公式 cardqa_op_10: リーダーが超新星を持たなくても **ライフには加えられる** = 前半は無条件)。

### conform (18 件、 要点)

- 「捨てたカード1枚につき、 リーダーかキャラ **1枚** は +1000」 = 加算先は単一 (OP06-014)
- 「バトルした場合」 は **キャラ同士のバトル成立時のみ** (ST02-010、 リーダーへのアタックでは不成立)
- **発動したイベント自身がトラッシュに入ってから** トラッシュ枚数条件を判定する
  (OP05-095 「15枚以上」 を14枚で / OP07-096 「10枚以上」 を9枚で → どちらも成立。 ⚠ 実経路
  `PlayEvent` で測ること。 `trigger_main_event` を直接呼ぶとカードがトラッシュへ行かず誤判定する)
- replace_leave の対象に **自身** を含む (OP15-035) / 任意コストに **自分自身** を選べる (OP07-085)
- 【トリガー】での発動は **イベント発動ではない** (OP15-002 の起動メイン条件を満たさない)
- 「効果を無効にする」 を適用されたキャラは 「元々の効果がない」 に **含まれない** (印刷テキスト基準)
- 「その後 相手はカード1枚を引く」 分は **公開されない** (OP07-090)
- トラッシュのイベントの【メイン】を発動しても **カードはトラッシュから動かない** (EB03-031)
- 任意コストの filter は公式どおり厳密 (OP03-070 は **コスト5ちょうど** = 4以下では発動不可)
- 第1ターン目は【速攻】を得ても **アタックできない** (PRB01-001)
- 「自分の効果」 = **そのカードをリーダーにしているプレイヤー** の効果 (OP14-079 クロコダイル)
- 「その後」 の独立 do は前半が0枚でも走る (OP16-067 の手札捨て / OP15-077 のドロー)
- `draw_to_hand_size` = 手札が N 枚になるように引く (OP02-069)

## 2026-08-11 #7 — 節 (clause) 単位の overlay カバレッジ監査を新設 + 実バグ 3 件

### なぜ作ったか

2026-08-11 の pending 消化で 「公式テキストの **後半の節** が overlay から丸ごと落ちている」 型が
**4 回** 出た (OP10-116 の scry / OP04-081・091 の mill / OP10-119・OP12-031 のドン付与 /
OP08-114 の静的効果)。 毎回 「掃引すると 2 枚だけ」 で孤立していたが、 **同じ形の見落としが
繰り返し出る** ので個別に見つけるのをやめ、 検出器を作った。

既存 `audit_text_overlay_consistency.py` は 「テキストに動作語が有るのに primitive が無い」 を
**カード単位** で見るため、 「A。 **その後**、 B」 の B だけが落ちても A の primitive がカードの
どこかに有れば flag されない。 実測: 上記 7 枚のうち **2 枚しか flag しなかった** (しかも別理由)。

### `scripts/audit_text_clause_coverage.py`

公式テキストを 【マーカー】 で効果ブロックに割り → 「。」 と 「その後、」 で **節** に割り →
各節の動作語から期待 primitive family を決め → その when の overlay entry の primitive キーを
**入れ子ごと** (do / cost / optional_cost_then / conditional / choice_effect / then / effect) 集めて
突合する。 動作語が **1 種類だけ確実に取れる節** に限って判定する (誤検出を減らす)。

**校正**: 修正前の overlay (`git show 02683c8:db/card_effects.json`) に掛けて、 既知の **9 件を
全部検出** できることを確認してから使った (`--overlay` オプション)。 修正後は 0 件になることも確認。

**誤検出の削り方** (実測ベース):
- family キーの取りこぼしを補完 (`search_top_n(destination=life)` / `chara_to_opp_life` /
  `view_life_top_choose_position`)
- `fire_self_effect` 等 「他の効果へ委譲する」 primitive を持つ entry は **判定不能として skip**
- ⚠ **マーカー無しの 「〜した時、」 散文トリガー** は when を一意に決められない
  (on_ko / opp_event_played / on_self_don_returned_to_deck …)。 ここだけカード全体スコープに
  戻す。 外すと flag が **55 → 151** に膨らみ、 増分はほぼ全部これだった。

現行 overlay での flag = **59 件 / 57 枚** (family 別: play 22 / power 12 / to_deck_bottom 4 /
cost_mod 4 / draw 4 / add_don 3 / KO 3 / attach_don 2 / untap 2 / 他)。

### 【fixed】カウンターイベント 3 枚 — 「その後」 の **本体除去が丸ごと欠落**

監査の初回実行で即座に出た。 いずれも overlay に **power_pump しか無かった**:

- **ST06-014 衝撃波**: 「その後、 相手の **アクティブの** コスト3以下のキャラ1枚までを、 KOする」
- **OP07-055 蛇ダンス**: 「その後、 自分のキャラ1枚までを、 持ち主の手札に戻す」
- **OP07-094 剃**: 「その後、 自分のトラッシュが10枚以上ある場合、 自分の特徴『CP』を含むキャラ
  1枚までを、 持ち主の手札に戻す」

**カウンターを切っても除去が一切起きない** = 実戦の勝敗に直結する欠落。
実測で公式どおりになることを確認 (ST06-014 は **レストのキャラを誤って KO しない** ことも確認、
OP07-094 は トラッシュ9枚では起きず10枚で起きる)。

## 2026-08-11 #8 — 節カバレッジ監査を flag 0 まで消化 (実バグ 5 枚追加、 累計 8 枚)

`scripts/audit_text_clause_coverage.py` の flag を **59 → 0** まで下げた。 内訳は
**誤検出の削り込み** と **実バグの修正** の両方。 ⚠ 削るたびに 「修正前 overlay で既知 9 件を
検出できるか」 を再校正し、 **9/9 を維持** した (= 検出力を落として 0 にしていない)。

### 誤検出の削り方 (family キー / 構造の取りこぼし)

- **【ドン‼×N】 は when ではなく gate**。 単独で when を決めると
  「【ドン‼×1】【ターン1回】自分がダメージを受けた時…引く」 のような散文トリガーを
  on_attached_don entry で探して誤検出する → 静的扱い (カード全体スコープ) に。
- **【マーカー】が 「を持つ」 に続く場合は参照** (「【トリガー】を持つカード」)。 除外しないと
  【登場時】の本文が when=trigger に割り当てられる。
- family キーの取りこぼし補完: `search_top_n(destination=play/life)` /
  `reveal_life_top_play` / `summon_stage_from_deck_with_feature` /
  `optional_discard_hand_for_battle_buff` / `rest_self_don_for_battle_buff_per_don` /
  `return_self_charas_then_pump_per` / `opp_may_return_active_don_else_debuff` /
  `reduce_play_cost` / `return_self_chara_to_hand` / `search` 等。
- `fire_self_effect` 等 「他の効果へ委譲する」 primitive を持つ entry は **判定不能として skip**。

### 【fixed】静的コスト修正の欠落 3 枚

- **OP15-088 パイレーツドッキング6** 「このキャラのコスト+6」 (印刷5 → 実効11 のはずが 5 のまま)
- **OP12-042 アルビダ** 「自分の**元々の**コスト5以上のキャラが2枚以上いる場合、 このキャラのコスト+1」
- **ST23-002 シャンクス** 「手札のこのカードは、 相手の**元々の**パワー8000以上のキャラがいる場合、
  コスト-3」 → 条件 `exists_opp_chara_truly_original_power_ge` を新設 (Python / Rust)。
  ⚠ 「元々の」 なので **書き換え後の元々のパワー** で判定する (公式 4-9-2-1)。

### 【fixed】OP07-078 メガトン九尾ラッシュ — primitive が **別物** だった

【トリガー】「ドン‼**デッキから**ドン‼1枚までを、 **アクティブで追加する**」 に対し overlay は
`untap_don` (= **場にある** レストのドンをアクティブにする) を使っており、 **ドンが増えなかった**。
→ `add_don` に是正。 ⚠ 「欠落」 ではなく **意味の違う primitive を当てていた** 型。

### 【fixed】OP07-091 モンキー・D・ルフィ — 欠落 + **固定値パンプ**

公式: 「相手のコスト2以下のキャラ1枚までをトラッシュに置く。 **その後、 自分のトラッシュから
コスト4以上のキャラカードを任意の枚数好きな順番でデッキの下に置く。 置いた枚数3枚につき**、
このキャラは、 このターン中、 パワー+1000。」
overlay は **トラッシュ→デッキ下が丸ごと欠落** し、 パンプも 「置いた枚数に依らない固定 +1000」
だった (= **0 枚しか置けなくても +1000** されていた)。
→ 専用 primitive `trash_to_deck_bottom_then_pump_per` を新設 (Python / Rust)。
実測: 2枚→+0 / 3枚→+1000 / 7枚→+2000、 対象外 (コスト4未満) は動かさない。
⚠ 「任意の枚数」 の選択は本来プレイヤーの判断。 AI は該当カードを全部置く (= パンプ最大化) を既定にした。

### ⚠ 監査ツールの footgun (踏んだので記録)

`--card` / `--overlay` (= 校正・調査用) の実行が **成果物 JSON を上書き** していた。
校正ループを回した直後に成果物を読むと 「flag 1 件」 の中身が **旧 overlay の 1 枚ぶん** に
差し替わっており、 危うく誤読するところだった → 調査用実行では書き込まないようにした。

## 2026-08-11 #9 — 【ダブルアタック】でも 「ダメージを与えた時」 は 1 回 (escalated 解消)

### 【fixed】8ccd4a205279 — attacker 側 when が hit ごとに発火していた

- **一次情報** (cardqa_op_03): 「この 『相手のライフにダメージを与えた時』 の効果は、
  【ダブルアタック】を持つキャラが相手のライフに2ダメージを与えた時に2回発動できますか？」
  → 「**いいえ、 できません。 1回のみこの効果を発動できます**」
- **違反**: engine は damage ループの **hit ごと** に attacker 側 when (`on_opp_life_taken`) を
  発火しており、 実測で **OP03-041 ウソップ が 7 枚 → 14 枚 mill** していた。
- **根拠**: 【ダブルアタック】は 「このカードが与えるダメージは2になる」 = **1 つのダメージ事象**。
  ライフが 2 枚離れるのはその結果なので、 「ダメージを与えた時」 は 1 回。
- **是正**: damage ループの hit index を見て 1 発目だけ発火 (Python `game.py` / Rust `rules.rs`)。
  ⚠ **defender 側** (「自分のライフが手札/トラッシュに移動した時」) は **カードごとの事象** なので
  毎 hit 発火のまま。
- **安全確認**: `when=on_opp_life_taken` を使う **7 枚全部の公式テキストを確認**。 6 枚が
...
## 2026-08-12 #11 — ライフを表/裏にするコストは 公式の **位置指定** どおりに払う (n/a 5 件を解消)

**背景**: 前日の per-card 化 ([2026-08-11 #10](#)) で 「どのライフが表向きか」 が表現できるように
なり、 それまで 「count モデルでは位置を表現できない」 という理由で **n/a に落としていた裁定**
が一気に検証可能になった。 台帳を洗い直して 5 件を消化した。

**engine の誤り**: `flip_life_face_up` / `flip_life_face_down` を **一律 「上から順に最初の該当」**
で実装していた。 公式テキストは 3 通りに書き分けている:

| 文面 | 意味 | 枚数 | spec |
|---|---|---|---|
| 「自分のライフの **上から** 1枚を…」 | **一番上のみ**。 既にその向きなら払えない | 23 | `pos:"top"` (既定) |
| 「自分のライフの **上か下から** 1枚を…」 | **両端のみ** | ST36-005 | `pos:"top_or_bottom"` |
| 「自分の **表向きのライフ** 1枚を…」 | **位置自由** | ST13-009 | `pos:"any"` |

### 一次情報と実測

- **0baebbf40f33 / ST20-001 カタクリ**: 「一番上が表向きの場合…付与することはできますか？」
  → 「**いいえ**」。 実測 上が裏 = 起動メイン 1 件 / **上が表 = 0 件** (従来は下段の裏向きを
  掴んで 1 件だった)。
- **5cf59becb545 / OP10-099 キッド・P-106 ルフィ** (【自分のターン終了時】): 同型。
  実測 上が裏 `[裏,表,表]→[表,表,表]` (発動) / **上が表 `[表,裏,裏]` = 変化なし**。
- **6896058e0308 / ST36-005 キッド**: 「一番上と一番下がどちらも表向きの場合、 発動できますか？」
  → 「**いいえ**」。 実測 `[表,裏,表]` = **0 件** (中段の裏向きを掴まない) / `[表,裏,裏]` = 1 件。
- **042e57595c08 / ST13-009 シャンクス**: 「好きな位置にある表向きのカードを裏向きに
  できますか？」 → 「**はい**」。 実測 表向きが **最下段だけ** `[裏,裏,表]→[裏,裏,裏]` かつ
  後文 (相手ライフ 3→2) が走る。 表向き 0 枚なら未払いで後文も不発。
- **0ef7dc982974 / ST13-004**: 並べ替えで表裏が保存される (= #10 (5) で是正済)。
- 既に conform だった **2d8cc0616fb0 / OP08-063** も、 従来は count が偶然一致していただけ。
  実測 上が表 = ドン+1 / **上が裏 = ドン±0** で 公式 「いいえ」 と一致する形に。

### overlay の付随是正

**OP08-058 シャーロット・プリン** 「自分のライフの **上から2枚** を表向きにできる」 を
`{上から1枚}×2` と書いていた。 位置指定 (= 一番上のみ) を入れると 2 個目が
「一番上は既に表向き」 で払えなくなるので `{"flip_life_face_up": {"count": 2}}` に畳んだ。
実測 上 2 枚とも裏 → `[表,表,裏]` + レストドン+1 / 上 1 枚が表 → 変化なし・ドン±0 /
ライフ 1 枚 → 変化なし。

### ⚠ 残す穴 (= 隠さず記録)

「上か下」 「位置自由」 で **どちらを選ぶか** は今のところ AI 決定的 (上優先) で、
**人間に選ばせる modal を配線していない** ([[feedback_human_ai_option_parity]] に対する既知の穴)。
ST13-009 は ST13-003 と同じデッキに入るので 「どの表向きを裏返すか」 は実際に意味を持つ。
`_flip_life_targets` の docstring に同じ注記を置いた。

### 検証

rust_parity_check 2131 / rust_parity_sweep 40430 / effect_smoke 5735 — bail 0・MISMATCH 0・
static_skip 0。 pytest 6029 passed。 ⚠ sweep の比較点が 41201 → 40430 に減っているのは
**払えなくなったコストのぶん legal action が消えた** ため (= 期待どおり)。

---

## FAQ conformance バッチ (2026-08-11、 cron optcg-faq-conformance): fixed 2 / conform 7 / n/a 5 / escalated 6

### fixed 1: OP15-028 ニャーバン兄弟 — 「コストエリアのドン」付与に from_cost_area 欠落 (タダ空振り)

**一次情報** (`cardqa_op_15`、 qid 93d5946f56d6):
> Q: この【登場時】効果で、 相手のアクティブのドン!!を付与することはできますか？
> A: はい、 できます。 **アクティブかレストかに関わらず、 コストエリアのドン‼を付与できます**。

OP15-028「相手のコストエリアのドン‼1枚まで」の overlay `attach_rested_don` が `from_cost_area`
を持たず、 相手の `don_rested` のみを source していた。 相手のコストエリアのドンが全て
**アクティブ** (= 自分のターン中、 相手が未消費) だと 0 枚しか付与できず **空振り**。
`from_cost_area: true` を追加 (rested 優先、 不足分を active から補充)。 primitive 側の
active 対応は Python/Rust 双方に実装済 (= overlay データのみの是正、 両engine同一挙動)。
**全走査**: テキストに「コストエリアのドン」を含み付与する card は OP15-023/025/028 のみで、
023/025 は既に `from_cost_area` あり = **取りこぼしは 028 の 1 枚のみ**。
ガード: `test_op15_028_attaches_from_active_don_in_cost_area` +
`test_cost_area_don_attach_cards_source_from_active_scan` (全走査)。

### fixed 2: OP14-120 クロコダイル — ドロー条件が「相手の」を無視し両陣営を数えていた

**一次情報** (`cardqa_op_14`、 qid 9493e614d556):
> Q: 相手の場にコスト10以上のキャラだけがいる場合、 この【登場時】効果でカード1枚を引くことはできますか？
> A: はい、 できます。

OP14-120「その後、 **相手の**コスト0か8以上のキャラがいる場合、 カード1枚を引く」は
**相手陣営のみ**を数える。 だが overlay は両陣営を数える `exists_chara_cost_0_or_ge_8` を
使っており、 **自分だけ**が cost0/≥8 キャラを持つ場合に (相手に該当が無くても) 引けてしまう
違反があった。 Q&A の cost10+ シナリオ自体は両条件で真なので元々 conform だが、 「相手の」の
書き分けを潰していた。 opp-only 条件 `exists_opp_chara_cost_0_or_ge_8` を Python
(`engine/effects.py`) と Rust (`rust_engine/src/effects.rs`) の両方に追加し、 OP14-120 と
OP14-120_p1 の overlay を差し替え。 ⚠ **OP14-090 (ダズ) / OP14-094 (Mr.5) は「相手の」修飾が
無く両陣営** = `exists_chara_cost_0_or_ge_8` のまま (= 書き分けを保存)。
ガード: `test_op14_120_draw_condition_counts_only_opponent_characters`
(OP14-090 が両陣営のままである対照も含む)。

### conform (公式どおり = 是正不要、 再調査回避のため記録、 2026-08-11)

- **OP12-015** (915e001b56b4): `self_attached_don_ge` は leader + 全キャラの attached_dons を
  **合計**。 異なるカード/リーダーに 1 枚ずつでも合計 2 以上を満たす = 公式どおり。
- **OP02 コスト-5** (92defbf8b259): 効果テキストの「キャラ」= 場のキャラのみ。 engine の
  コスト修正は場の InPlay にしか届かず手札/トラッシュには適用されない = 公式どおり。
- **OP05-007 サボ** (934f3077d8a5): `ko_total_power_le(cap 4000)` は単独 power 4000
  (合計 4000 ≤ 4000) を選択可。 「単独 power > cap」 のみ除外。 = 公式「パワー4000をKO可」どおり。
- **OP01-051 キッド 起動メイン** (93e4f42d7289): cost = `rest_self` (手札の対象と独立に払える)、
  do = `play_from_hand limit1` (「まで」= 任意)。 手札にコスト3以下が無くてもレスト可 = 公式どおり。
- **ST10 バトル中断** (95bdfadcc640): アタック中にブロッカー (= 被アタック対象) が【ターン1回】で
  KO されたらカウンター以降スキップしバトル終了 = バトル中断則 (2026-08-04 既決、 engine 正)。
- **OP06 無効化KO** (96e3be09bfde): 効果を無効にされ KO されたキャラの【KO時】は発動しない
  (既決、 engine 正)。
- **OP08-043 白ひげ** (927eba94fc0c): `set_attack_cost_discard_hand` は解決時の相手キャラ
  (`all_opponent_characters`) にのみフラグ付与。 次ターン以降に登場した速攻キャラは対象外
  = 手札 2 枚を捨てずにアタック可 = 公式どおり。

### n/a (engine 状態変化に落ちない、 2026-08-11)

- 918b1f562ebc OP15 起動メイン: 付与するドンをどちらが選ぶか = 手続き的選択。
- 934385ed5067 OP01 登場時: デッキ 4 枚以下でも発動可/閲覧中はデッキ 0 枚扱いにならない
  = デッキアウト敗北の判定タイミングの定義。 engine は解決中にデッキアウト判定しない。
- 93c561f6209e OP01: 「このキャラ」= 発動元 1 枚のみ、 の語の定義確認。
- 94ab3345bde5 OP16: デッキ下に置くカードは持ち主が自身の順で選ぶ = 手続き的選択/情報。
- 963f7e6c018a OP06: 相手キャラをデッキ下に置く順を自分は知れるか = 情報隠匿の確認。

### escalated (自動修正の範囲外、 2026-08-11、 要 focused 対応)

- **91f4570ec240** OP10 たしぎ + OP05-030 ロシナンテ の replace 連鎖 (場を離れる代わりにレスト/
  KO される代わりにトラッシュ)。 多カードの replace 相互作用で裁定確度が不足。
- **92f7b4efa60f** ST17-001 クロコダイル: `reveal_top_then` が公開カードをデッキから外し →
  `then`(draw 2) → 後で上に戻す順序のため、 公式「**公開カードを含めて**デッキの上から 2 枚を引く」
  とズレて公開カードの **下** 2 枚を引く。 = 実バグだが reveal-then-draw の順序修正は
  `reveal_top_then` + `draw` + `rest_remain:top` の全走査 + 両engine 対応が必要でバッチ範囲外。
- **93918a623eeb** OP01-051 キッド taunt (`set_attack_taunt`) と別カードの「アクティブキャラにも
  アタック可」付与の相互作用。 taunt が対象をキッドのみに制限し続けるかは多効果相互作用で確度不足。
- **944b097b8ac0** OP11-041 ナミ / OP12-099 カルガラ: 「ライフが離れた時」(= 自他両方) だが
  overlay は `on_self_life_to_hand` のみ配線 = **相手ライフ (自攻撃) で発火しない**。 実バグ。
  修正には `on_opp_life_taken` / `on_self_life_to_trash` 追加 + 【ターン1回】共有名前付きキー
  (when 横断で 1 回に束ねる、 OP13-002 型) + 両engine parity が必要。 2 件だが二重発火の機構リスク
  ありで focused 対応にする。
- **94f4a4a51b57** EB01-040 キュロス: 発動コスト「自分のライフの上から 1 枚を表向きにする」が
  overlay 未実装 (`cost` = `once_per_turn` のみ) = **タダ撃ち / 上が既に表向きでも撃てる**。
  全走査で「ライフ…表向きにする：」コストが未実装の card は **約 15 枚** (EB04-059/EB03-053/056/
  EB02-060/EB01-040/OP15-114/OP13-114/117/OP11-022/117/OP10-099/OP08-058/ST20-001/P-106 等)。
...
## 2026-08-12 #12 — escalated 6 件を全消化 (5 fixed / 1 conform)

cron `optcg-faq-conformance` が 「多カード相互作用で確度不足」 「機構リスクあり」 として
保留していた 6 件。 前日の per-card ライフ化 + 位置指定の実装で前提が揃ったものを含め、
**全件を一次情報つきで決着させた**。

### 【fixed】91f4570ec240 — 同時 KO の置換連鎖は 「持ち主が最も損しない順」 で解ける

- **Q** (cardqa_op_10): アクティブの OP10-032 たしぎ と OP05-030 ロシナンテ が同時に KO される時、
  たしぎ の置換で ロシナンテ を救い (たしぎ はレスト)、 さらに ロシナンテ の置換で
  **今レストになった たしぎ** を救えますか？
- **A**: 「**はい。 レストの『たしぎ』だけが場に残り、『ロシナンテ』はトラッシュ**」
- **違反**: engine は victim を **盤面順** で処理していた。 たしぎ が先だと
  「たしぎ が先にトラッシュ → ロシナンテ だけ助かる」 = **公式と真逆**。 実測で確認。
  = 公式の線が **盤面の並び次第で到達不能** だった。
- **是正**: `_order_simultaneous_victims` — 「バッチ内の **他の** victim を救える置換 holder」 を
  **後回し** にする (= 先に他を救い、 その結果 (レスト等) で自分も救われる)。
  `ko` / `ko_multi` / `ko_all_others` の 3 経路に配線。 実測で **並び順に依らず** 公式どおり。
- ⚠ **残す穴**: 本来は持ち主の **選択** (公式 「自分の効果は好きな順」)。 人間 modal は未配線で、
  ここでは 「最も損しない順」 を決定的に選んでいる。

### 【fixed】92f7b4efa60f — 「公開し」 はカードを動かさない

- **Q** (cardqa_st_17 / ST17-001 クロコダイル): デッキの上から1枚を公開して《王下七武海》だった場合、
  その公開したカードはどうなりますか？
- **A**: 「**公開したカードを含めて** デッキの上から2枚のカードを引き、 その後手札1枚をデッキの上に置きます」
- **違反**: `reveal_top_then` が公開カードを **先にデッキから抜いてから** then を実行していたので、
  「カード2枚を引く」 が公開カードの **下 2 枚** を引いていた。 同型 OP14-044 / ST22-003 / ST22-006。
- **是正**: 公開は in-place。 `rest_remain` の移動は then/else の **後**、 かつ
  **デッキ先頭がまだ公開カードのまま** の時だけ行う (= then が引いてしまった場合は何もしない)。
  実測 ST22-003: 手札 = `[公開カード, その次]`。 Rust も同形に。

### 【fixed】944b097b8ac0 — 「ライフが離れた時」 は自分/相手どちらでも

- **Q** (cardqa_op_11 / OP11-041 ナミ): この【自分のターン中】効果は、 自分のライフが離れた時にも
  発動できますか？ → **「はい。 自分のライフか相手のライフかにかかわらず」**
- **違反**: overlay が `on_self_life_to_hand` だけを配線しており、 **相手のライフが離れた時
  (= 自分のアタック) に発動しなかった**。 OP12-099 カルガラ も同型。
- **是正**: `on_opp_life_taken` / `on_self_life_to_trash` に複製。 【ターン1回】は
  **名前付き共有キー** (`"once_per_turn": "OP11-041_life_leave"`) で when 横断に束ねた。
  実測: アタックで手札 0→1 / 自ライフ→手札で +2 / 2 回目は共有ターン1回で +1 のみ。

### 【fixed】94f4a4a51b57 — EB01-040 キュロス の発動コストが overlay から欠落

- **Q** (cardqa_eb_01): 自分のライフの一番上が表向きの時、 この【起動メイン】効果で相手のコスト0の
  キャラ1枚を KO できますか？ → **「いいえ」**
- **違反**: 「自分のライフの上から1枚を表向きにできる：」 (= 発動コスト) が overlay に無く、
  **タダ撃ち** + 一番上が既に表向きでも撃てた。
- **是正**: `optional_cost_then` + `flip_life_face_up: {"pos": "top"}`。 実測 上が裏 → 起動メイン 1 件
  (表向き化 + コスト0キャラ KO) / 上が表 → **0 件・KO なし**。

### 【fixed】95129d358381 — 対象カードは OP02-004 ニューゲート

cron は 「OP02 内で対象を一意特定できず」 保留していた。 【登場時】に
「その後、 自分は、 このターン中、 自分の効果でライフを手札に加えられない」 を置くのは
**OP02-004 エドワード・ニューゲート** (OP02-023 は【メイン】イベント)。
- **A**: 「いいえ、 支払うことはできません。 **その発動コストを支払えないため、 効果の発動もできません**」
- **違反**: engine は payability にロックを見ておらず、 起動メインが発動できていた。
- **是正**: `_life_to_hand_cost_payable` / `_optional_cost_payable_in_do` にロックを配線。
  実測 ロック前 1 件 → ロック後 **0 件・札も動かない**。
  ⚠ ST13-003 の置換 (= 札は動くが未払い) と違い、 こちらは **札が動かない**。

### 【conform】93918a623eeb — taunt は 「アクティブにもアタック可」 でも解けない

OP01-051 キッド の 「キャラの『ユースタス・キッド』以外にアタックできない」 は、
OP11-010 ひばり の 「アクティブのキャラにもアタックできる」 を得ても解けない (公式 「いいえ」)。
実測: taunt 有 + アクティブ可 → リーダーのアタック候補 = **[キッドのみ]** /
taunt 無 → [相手リーダー, キッド, ナミ]。 是正不要。

### ⚠ 掃引の穴: たしぎ×ロシナンテ は **合成デッキ 329 個のどれにも同居しない**

`_order_simultaneous_victims` を Python だけに入れた直後、 `rust_parity_sweep` は
**match 40394 / MISMATCH 0** で緑だった。 しかし合成デッキを調べると
**OP10-032 と OP05-030 が同居するデッキは 0 個** = この経路は掃引に載っていない。
該当局面を直接 Python/Rust で突き合わせて **silent MISMATCH を確認**し、 Rust に 2 件の是正:

1. **victim 並べ替えの未追従** — `order_simultaneous_victims` を `ko` / `ko_multi` に移植。
   ⚠ Python は **免疫フィルタの前** に並べ替えるので、 Rust も同じ位置に置く。
2. **`replace_ko_match` が `target_rested` を読んでいなかった** (独立バグ)。
   OP05-030 「自分の **レストの** キャラがKOされる場合」 が **アクティブの victim にも一致**
   していた。 Python は読んでいたので **片側だけの誤り** = 差分検証で見えるはずだったが、
   掃引に該当ペアが無く沈黙していた。 回帰: `test_replace_ko_target_rested_is_honoured`。

`ko_multi` 経路は Rust が 「複数 victim × cascade」 で **明示 bail** するので乖離ではない。
⚠ 計器の注意: `apply_raw_effect_digest` は `execute_effect` の戻り値を捨てるので、
**bail と MISMATCH を区別できない**。 bail 判定には `apply_action_digest` (= Err が伝播する)
を使うこと。 これを見落として一度 「ko_multi も MISMATCH」 と誤読した。

> 教訓: **掃引が緑でも 「その組合せが載っているか」 は別問題**。 カード単位のカバレッジ
> (= 全カードがどこかの合成デッキに載る) は **カード間相互作用のカバレッジではない**。
> 相互作用の裁定を直したら、 その 2 枚を並べた **狙い撃ちの差分テスト** を必ず 1 本書く。
...
## 2026-08-12 #13 — pending 消化バッチ (台帳末尾から 16 件、 1 fixed / 14 conform / 1 n/a)

`--from-end` の手動バッチ (cron は先頭から)。 実測で公式と突き合わせ、 **違反は 1 件**。

### 【fixed】ea8873b15c59 — OP12-017 「赤の」 は or の **両方に係る**

- **Q** (cardqa_op_12): この【メイン】効果で、 **赤以外の**コスト3以上のキャラカード1枚を
  手札に加えることはできますか？
- **A**: 「**いいえ、 できません。 この効果は、 『赤のイベント』か『赤のコスト3以上のキャラ
  カード』を手札に加えることができる効果です**」
- **違反**: overlay の `or_clauses` が
  `[{EVENT, 赤}, {CHARACTER, cost_ge:3}]` = **キャラ側に色が無く、 青などのコスト3以上キャラを
  拾えていた**。 実測で 青コスト4 が手札に入った。
- **是正**: キャラ側にも `"color": "赤"`。 実測 是正後は 青 = 加えない / 赤コスト4・赤イベント = 加える。
- ⚠ 同型を全走査したが、 他の 「A か (色)のイベント」 型 5 枚 (OP07-013 / OP12-006 / OP12-014 /
  OP12-028 / OP12-034) は **色が片方だけで正しい** (公式テキストがそう書いてある) ので触らない。
- 既存テスト **3 本** が **旧 filter を正解として固定** していたので同時に是正
  (= backfill がバグを固定する例、 これで累計 4 例目):
  `test_op12_017_search_filter_matches_official_or_clause` (or_clauses の literal 比較) と、
  `test_op12_017_main_search_ai` / `..._human_pick` (= サーチ先に **緑の** シルバーズ・レイリー
  ST32-004 を置いて 「引けること」 を assert していた)。 後者は赤のコスト4キャラ (EB02-002 サボ)
  をサーチ先に変更。 レイリーはコスト支払い用に場へ置くだけなので色は無関係。

### 【conform】主なもの (すべて実測で確認)

- **ec5175486625 / EB04-010**: 「パワー0にする」 キャラにカウンター +1000 → **1000** になる
  (0 は base 上書き、 カウンターは battle_buff 加算で上に乗る)。
- **ec136a9b843e / OP15-088**: コロン前が発動コストなので、 **コストで落とした 3 枚** から登場できる。
- **eac0802ada51 / OP14-079**: コストで KO した自キャラの【KO時】は **本体の後**
  (公式 8-4-1-3〜5、 2026-08-09 の是正が効いている)。 実測ログ順 = 自KO → コスト-10 → mill2 → ドロー1。
  ⚠ **計器注意**: modal 保留中に `resolve_triggers` を呼ぶと順序が逆に見える (artifact)。
- **eb51009012b3 / OP01-073**: デッキ上5枚は **まとめて**上か下 (1枚上+4枚下 の分割は不可)。
- **edb996f82df7 / ST12-001**: 素の 「パワー7000以下」 は **現在パワー** (印刷7000+ドン1=8000 は不可)。
- **ed55b76c406b**: 「自分の場にドン!!が8枚以上」 は **付与ドンも数える** (`self_don_ge`)。
- **ecb81075e8c6**: 【自分のターン終了時】は 他の【自分のターン終了時】の結果を参照できる
  (2 相モデル = 全部のコスト → 全部の do)。 ⚠ do 順は盤面順で、 **持ち主が順を選ぶ modal は未配線**。
- **edeae8512efb / OP13-079**: 「ゲーム開始時」 は **手札を準備する前** + 探した後にデッキを再シャッフル。

---

## 2026-08-12 #14 — 公式 Q&A の **カード紐付け** を取りこぼしていた (スクレイパ是正)

**発端** (ohtsuki 指摘): 「相手のライフがない場合、 この【登場時】効果でゲームに勝利できますか？」
を 「対象カードを特定できない」 として n/a にしたが、 公式ページには
**Q304 / P-009 トラファルガー・ロー** と明記されている。

**原因**: `scripts/scrape_official_faq.py` の `parse_qa_areas` が `div.qaArea` (= Q/A 本文) だけを
読んでおり、 その親 `div.resultItem > div.titleArea` にある
`dt.qaNum` (Q304) / `dd.qaTit` (**P-009 トラファルガー・ロー**) / `dd.qaDate` を **捨てていた**。

→ `resultItem` 起点に書き換え、 `q_no` / `title` / `card_id` / `updated` を保持する
(`_CARD_ID_RE` で qaTit 先頭のカード番号を抽出)。 旧レイアウト用の fallback も残す。
`faq_qa_manifest.py` は card_id を台帳へ持ち回り、 `--next` の表示にも出す。
committed snapshot (`db/cardqa_tagged.json`、 = 一次が gitignore なのでクラウド cron の
fallback 元) にも載せる。

**効果 (実測)**: 全 series 再取得後、 cardqa 2,682 件のうち **1,341 件 (50.0%)** に card_id。
50% ちょうどなのは公式ページが同じ Q&A を 「シリーズ名」 見出しと 「カード名」 見出しの
**2 回** 載せるため (= どの Q&A にもカード付きの双子がある)。
→ **台帳 1,205 件すべてに card_id が付き、 未処理 372 件も 100% カード特定可能** になった。
台帳の件数・処理済 status は不変 (消失 0)。

> 教訓: 「特定できない」 と判断する前に **一次ソースの取り込みで落としていないか** を疑う。
> cron が escalate した OP02 の 1 件も、 私が n/a にした P-009 の 1 件も、 engine ではなく
> **スクレイパの欠落**が原因だった。

### 【conform】ea61ef0eb1ee — P-009 トラファルガー・ロー (n/a を撤回)

【登場時】相手の手札が6枚以上ある場合、 相手は自身のライフ1枚を手札に加える。
相手のライフが 0 枚なら効果は何も起きず、 **勝利にもならない**
(ライフ 0 は 「ダメージを受けた時」 に敗北するので、 効果でライフを取れないだけでは負けない)。
実測: 相手ライフ3枚 → 2枚 / 手札 6→7 枚。 相手ライフ0枚 → `game_over=False winner=None`、 手札 6 枚のまま。

---

## 2026-08-12 #15 — 「ライフの上に加える」 を engine は一貫して **一番下** に置いていた

pending バッチ (ST07-001) を実測する過程で見つけた **系統的な近似**。 該当する公式テキストは
すべて 「ライフの **上** に加える」 なのに、 engine は `life.append(...)` = **一番下** だった。

| primitive | 該当枚数 | 公式文面 |
|---|---|---|
| `put_top_to_life` | **50 枚** | 「自分のデッキの上から N 枚(まで)を、 ライフの上に加える」 |
| `hand_to_self_life` | **13 枚** | 「自分の手札1枚までを、 ライフの上に加える」 |

⚠ `put_top_to_life` にはコード内に 「ライフ上 (技術的には先頭追加だが簡略)」 と
**近似であることが明記** されていた ([[project_approximation_hides_bugs]] の型)。

**なぜ効くか**: 次のダメージで最初に離れるのは **上** の札。 どの【トリガー】が公開されるか /
ST13-003 のルール置換 / ST13-002 の 「表向きライフ全トラッシュ」 / ライフ mill の順が全部変わる。

### 併せて: 「**表向きで**加える」 が反映されていなかった

`hand_to_self_life` は常に裏向きで加えていたが、 公式テキストが 「ライフの上に **表向きで**
加える」 と書いているカードが **7 枚** ある (EB03-059 / EB04-060 / OP07-097 / OP08-116 /
OP09-104 / OP10-103 / OP10-107)。 overlay に `face_up: true` を配線した。
「裏向きで加える」 と明記する OP10-119 / ST13-005 は既定 (false) のまま。

実測: `put_top_to_life 2` → ライフ `[取1, 取2, 旧, 旧]` (上に、 取った順) /
`hand_to_self_life {face_up:true}` → `flags [True, False, False]`。
回帰: `test_put_top_to_life_goes_on_top_not_bottom` /
`test_hand_to_self_life_top_and_face_up_flag` /
`test_face_up_overlay_matches_official_text` (= overlay の face_up と公式文面の 1:1 を検査)。

⚠ **テスト 2 本が近似を正解として固定** していたので同時に是正
(`test_st36_001_cavendish_on_ko_ai` / `test_st36_002_killer_on_play_with_kid_leader` が
`life[-1]` を assert していた)。 これで 「backfill がバグを固定する」 例は累計 5 件目。

---

## 2026-08-12 #16 — 公式ページのカード番号は **2 件間違っている** (名前で突合して検出)

#14 で card_id を取れるようにしたが、 **番号を鵜呑みにすると別のカードを見て裁定する**。
実際に cron の裁定 (OP15-098/105) を検証しようとして 公式ページの 「OP15-106 ジュエリー・
ボニー」 を信じ、 cards.json の OP15-106 = **タコバルーン (効果なし)** を測って
「効果が無い」 と誤読しかけた。 正しくは **OP15-105 ジュエリー・ボニー** で、
**cron の裁定の方が正しかった**。

→ `scripts/verify_cardqa_card_ids.py` を新設。 公式ページの qaTit は 「番号 + 名前」 なので
**名前で突合** すれば番号誤りを検出できる。 実測 **名前一致 1,339 件 / 不一致 2 件**:

| 公式ページ | 実体 (cards.json) |
|---|---|
| OP15-106 ジュエリー・ボニー | **OP15-105** (OP15-106 は タコバルーン) |
| ST29-012 ロブ・ルッチ | **ST29-013** (ST29-012 は モンキー・D・ルフィ) |

`KNOWN_PAGE_TYPOS` に登録し、 未知の不一致が出たら `--assert` で落ちる。

### 台帳は `card_ids` (**リスト**) で持つ

同じ文面の Q&A が **複数カードにぶら下がる** (最大 10 枚)。 台帳キーは文面ハッシュなので
1 件に畳まれる → 先頭 1 枚だけ採ると **別カードの裁定を見て誤判定** する。
分布: 1 枚 = 1,123 件 / 2 枚 = 54 / 3 枚 = 18 / 4 枚 = 3 / 5 枚 = 5 / 7 枚 = 1 / 10 枚 = 1。

### 副産物: cards.json に **P-081 / P-082 の基本版が無い**

パラレル (`_p1` / `_p2` / `_r1`) しか無い。 テキストは同一なので裁定には影響しないが、
カード DB の欠落として記録する。

### 【conform】9cdec432f0d0 — cron が **古い前提** で n/a にしていた 1 件

ST13-017「火炎」竜王。 理由は 「engine は face_up_life_count(枚数)のみで物理位置の表裏を
持たない」 だったが、 これは **2026-08-11 の per-card 化で解消済** だった。
実測 前 `[(025,表),(016,裏),(013,表)]` → 後 `[(013,表),(025,表),(016,裏)]` =
順番は変わり表裏は札に追随 → conform。
...
## 2026-08-12 #19 — pending 消化バッチ (末尾から 6 件、 全 conform)

card_id が台帳に載ったので **カード特定が即座** にでき、 バッチの速度が上がった (#14)。

- **e7d90df22706 / OP02-027 イヌアラシ**: 「相手の効果で場を離れない」 は KO だけでなく
  手札 / デッキ / トラッシュへの移動も防ぐ。 実測 4 経路すべてで場に残る。
- **e7a6e74202a2 / OP08-105 ボニー**: 【トリガー】が **先**、 その後 (キャラが場に残っていれば)
  「相手のライフが離れた時」。 実測ログ順 = TRIGGER(1ドロー) → trigger→trash →
  ボニー(2ドロー + 手札1捨て)。
- **e76f9498b022 / OP06-044 ギオン**: 【トリガー】の発動は 「イベントの発動」 ではない。
  実測 相手手札 3→4・デッキ 20→19 (= トリガー自身の 1 ドローのみ、 ギオンは不発)。
- **e769ed62c947 / OP08-014 ワポル**: 「1枚まで」 なので相手キャラ 0 でも前半が空振りするだけで
  後半の自己 +2000 は成立。 実測 7000→9000 (相手キャラ 0/1 とも)。
- **e75420cdf0d4 / OP03-012 ティーチ**: 「**トラッシュに置く**」 は KO ではないので
  置いたキャラの【KO時】は発動しない。 実測 無条件【KO時】1ドロー持ち (OP15-012 バギー) を
  コストにしても 手札 0→1 (= コストのドロー 1 のみ)。
  ⚠ overlay のキー名は `ko_self_chara` だが **on_ko を発火しない除去** で、 KO 扱いの
  `ko_self_with_filter` (OP14-079 クロコダイル 「KOできる：」 = 【KO時】が発動する) とは
...
## 2026-08-12 #20 — pending 消化バッチ (末尾から 6 件、 全 conform)

- **e71370e3a7ce / OP15-033**: 「その後」 が空振りしても前半は成立。 実測 ライフ 0 でもリーダーは
  アクティブになる。
- **e62e1f8707c4 / OP14-001**: 「**元々のパワー**を入れ替える」 は base_power の入れ替えで、
  別効果の -2000 は残る。 実測 PRB02-002 ロー (印刷 7000、 自身の効果で -2000) と
  OP14-013 ルフィ (印刷 0) を入れ替え → ロー = **-2000** / ルフィ = 7000。 公式と一致。
- **e5c367da920e / ST11-004**: 「1枚まで」 なので手札に加えなくても後半 (ドン 1 アクティブ) は成立。
- **e5b7d94a758e / ST01-004**: 登場ターンに他の効果でレストになったらアタックできない。
  実測 アタック候補 1 → レストで 0。
...
## 2026-08-12 #21 — pending 消化バッチ (末尾から 4 件、 全 conform)

- **e5a6e173276a / OP10-110**: 【トリガー】の 「自分のライフが2枚以下」 に **このカード自身は
  含まない** (トリガーはライフから離れた後に解決する)。 実測 ライフ 3 枚 (離脱後 2) → 登場 /
  4 枚 (離脱後 3) → 登場しない。
- **e55db376af40 / ST32-005**: 【速攻：キャラ】でも **自分の 1 ターン目はアタックできない**
  (1 ターン目のアタック禁止が優先)。 実測 レストの相手キャラを対象に、
  ターン3 + 召喚酔いあり (= 登場したターン) → アタック候補 **1** (= 速攻：キャラ が効いている) /
  ターン1 → **0**。
- **e50d9a59ee0d / OP04-024**: 相手の【登場時】で **KO された** キャラは【相手のターン中】効果を
  発動できない。 engine の field-when は enqueue 時に 「場に居るカード」 をカード単位で積むので、
  場を離れたキャラは反応集合に入らない。
- **e56c5f4de6cf / ST01-016**: 「【ブロッカー】と **テキストに書いてあるだけ**」 のキャラは
  【ブロッカー】を持たないので KO 対象外。 engine の filter は **キーワード保有** で判定し
  テキスト文字列は見ない。

---

## 2026-08-12 #22 — 「自身の**場の**ドン‼」 には **付与されたドン** も含む (escalated 解消)

- **Q** (cardqa_op_02 / OP02-089・090・091): この【トリガー】効果で相手は
  **キャラやリーダーに付与された**ドン!!を戻すことはできますか？ → 「**はい、 できます**」
- **違反**: `return_opp_don` が **コストエリア (自由プール) しか減らしていなかった**。
  相手の自由ドンが規定枚数に満たない盤面では **規定枚数を戻せない** = 公式の 「できます」 を
  再現できない。 ⚠ `attach_don` は `don_active` → `attached_dons` へ **移動** するので、
  付与済のドンは自由プールに居ない。
- **是正**: 付与ドアへの fallback を Python / Rust 両方に実装。 選ぶのは **相手** なので
  損の小さい順に返す: ① レスト ② アクティブ ③ 付与 (付与の中は **キャラ → リーダー**、
  リーダーの +1000 を最後まで残す)。
- **実測**: 自由 0 + キャラ付与 1 → キャラの付与が 1 減りドンデッキ +1 /
  自由 0 + リーダー付与のみ → リーダーから / 場にドン 0 → 戻せない /
  レストがあるならレストが先。
- 回帰: `test_return_opp_don_can_take_attached_don`

> cron はこれを 「両 engine の対応 + chooser modeling が要り、 当環境は maturin 不在で
> Rust ビルド不可」 として escalated にしていた。 ローカルは maturin があるので両側を実装できた。

---

## 2026-08-13 #23 — 置換の 「代わりに◯◯できる」 は **代償**。 実行できなければ選べない

### 【fixed】e424a1e0c03a — OP13-109 ジュエリー・ボニー

- **Q** (cardqa_op_13): 自分のライフの一番上が **表向き** の場合、 このキャラが相手の効果で
  場を離れる代わりに自分のライフの上から1枚を表向きにできますか？ → 「**いいえ、 できません**」
- **違反**: overlay が `flip_life_face_up_effect` を **`do`** (= 置換の結果) に置いていたため、
  一番上が既に表向きでも **置換が成立して KO を免れて** いた。
- **是正**: cost の `flip_life_face_up {"pos": "top"}` に移し、
  `_can_pay_replace_cost` / `_pay_replace_cost` に flip 系を配線。
  実測 上が裏向き → 場に残る (上が表向きに) / **上が表向き → KO される** / ライフ0 → KO される。
- 回帰: `test_op13_109_replace_needs_face_down_top_life`

> **一般則**: 置換の 「代わりに **〜できる**」 は **代償**。 「その代償を実行できるか」 が
> 置換を選べる条件になる。 do に置くと 「代償を払えないのに置換が成立する」 = タダで KO を
> 免れる。 同型を触る時は必ず cost 側に置く。

### ⚠ Rust は明示 bail では済まない (テストが bail 0 を要求している)

最初 Rust 側を 「replace cost flip_life 未対応」 の明示 bail にしたが、
`tests/test_rust_effect_smoke.py` が **`bail == 0`** (= 全カード効果が Rust で実行できること)
を assert しており失敗した。 この guard は 「未実装を放置しない」 ための線なので、
逃げずに **Rust にも実装** した (解析 → `flip_life_targets` で payability → `flip_life_pay`。
同時離脱の `batch_already_paid` リセットにも追加)。

⚠ backfill テスト 1 本が `do` 側を見ていたので cost 側を見る形に是正
(= 「テストが旧実装を固定していた」 例は累計 6 件)。

### 【conform】同バッチの 4 件

- **e4725cc15633 / EB01-051**: デッキ 1 枚以下ではコスト (上 2 枚トラッシュ) を払えず KO もできない。
  実測 デッキ5 → KO / デッキ1・0 → KO なし・デッキも trash も不変。
- **e478c391b35f / OP15-035**: 2 枚同時に離れても **カード 4 枚をレストにするのではない**。
  同時離脱は 1 事象 = holder ごとに支払いは 1 回 (公式 cardqa_op_15、 OP15-090 と同型)。
- **e4f2c7b4f617 / OP04-020**: 【自分のターン終了時】は他の EOT 効果の結果 (アクティブになった
  ドン) をコストに使える (2 相モデル)。
- **e478edfe9231 / ST27-005**: 「相手の」 修飾が無い 「コスト3以下のキャラ」 = **両陣営** =
  自分のキャラも KO できる。

---

## 2026-08-13 #24 — 「手札 N 枚**まで**を捨てる」 で **0 枚** を選べない (escalated、 要 primitive 新設)

- **Q** (cardqa_op_02 / OP02-059 ハンコック・OP02-070 イワンコフ): 「その後、 自分の手札3枚までを
  捨てる。」 の効果で、 捨てる枚数に **0 枚** を選ぶことはできますか？
  → 「**はい、 できます。 0 枚から 3 枚までのうち好きな枚数**」
- **違反**: overlay に `[approx: 手札3枚まで=最大3として trash_self_hand_random:3]` と
  **近似が明記** されており、 実測 手札 5 → 2 = **常に 3 枚強制**。 0 枚を選べない。
- **未是正の理由**: 「0〜N から枚数を選ぶ」 primitive の新設 (人間 modal + AI の枚数選択) と
  Rust ミラーが要る。 当セッションの残りコンテキストでは **検証まで到達できない** ので
  半端に入れず escalated にした。
- **同型の全走査** (次に着手する人へ): 「手札 N 枚までを捨てる」 は **3 枚**
  (OP02-059 / OP02-070 / OP09-059)。 approx 明記があるのは OP02-059 のみだが、
  残り 2 枚も同じ primitive を使っているなら同じ穴。 **3 枚まとめて直すこと**。

### 【conform】同バッチ

- **e3ee7b1505df / OP10-022**: 「自分のキャラのコストの合計が5以上の場合」 の gate が効いている
...
## 2026-08-13 #25 — 「リーダーと**キャラ**1枚まで」 は **独立した 2 枠** (escalated 2 件のうち 1 件を解消)

### 【fixed】a8b1eab174ba — OP07-059 フォクシー

- **Q** (cardqa_op_07): この【アタック時】効果で、 相手のレストのリーダーとレストのキャラ
  **それぞれ1枚ずつ、 合計2枚** を選ぶことはできますか？ → 「**はい、 できます**」
- **違反**: `keep_opp_rested_inplay_next_refresh` が 「レストのリーダー + キャラ から **1 枚だけ**」
  の単一選択でモデル化されており、 **リーダーを選ぶとキャラを選べなかった** (= 合計 1 枚)。
- **是正**: リーダー枠 (1 まで) と キャラ枠 (limit まで) を **独立** に埋める (Python / Rust 両方)。
  人間 modal の上限も `1 + limit` に。 実測 リーダー+キャラともレスト →
  `(True, [True, False])` = **計 2 枚**。
- 回帰: `test_keep_opp_rested_picks_leader_and_character_independently`

### 【conform】a9149a4ed513 — 同カード

「相手のリーダーがアクティブの場合、 レストのキャラ1枚を選べますか？」 → 「はい」。
これは **是正前から conform** だった (単一選択でもキャラ側は選べていた)。
= cron は 2 件まとめて escalated にしていたが、 **実際に壊れていたのは 1 件だけ** だった。

> 教訓: 「同じカードの複数 Q&A」 をまとめて escalated にすると、 実際には片方だけが違反という
> ことがある。 **1 件ずつ実測して切り分ける**。
...
## overlay に **余分な primitive** が残っていた (2026-08-13 是正、 OP01-047)

OP01-047 ロー【登場時】「自分のキャラ1枚を持ち主の手札に戻すことが**できる**：自分の手札から
コスト3以下のキャラカード1枚までを、登場させる」。

overlay の `do` は `optional_cost_then` (cost = 自キャラ1枚を手札へ / effect = 手札から登場) だけで
足りるのに、 **その後ろに素の `return_to_hand` が残っていた**。 結果 バウンスが 2 回起き、
公式に無い 「もう 1 枚を無償で手札に戻す」 効果が付いていた (パラレル込み 5 エントリ)。

⭐ **教訓**: 「A できる：B」 を `optional_cost_then` に書き換えた時、 **元の A を do から消し忘れる**
と二重に効く。 cost へ移した primitive が do 側に残っていないか確認する。

## 公式どおりで **問題なかった** もの (2026-08-13 バッチ 4)

- **e0bcdb57fbe6** ST10-001: 「〜1枚**まで**をデッキ下に置き、 〜1枚**まで**を登場させる」 の 2 節は独立。
  対象が居なくても登場側は走る。
- **e03e4fa3c38e** OP03-047: 【登場時】の 「1枚まで戻す」 も 「置いてもよい」 も人間は見送れる
  (target_pick + optional_cost_confirm の 2 modal)。
- **dfb4682d0bc0** OP02-110: 【ブロック時】で **今アタック中のキャラ** を選んでも、 宣言済のバトルは
  中断されない (`set_cannot_attack` は以後のアタックを止めるだけ)。
...
## 「次に登場させる〈filter〉の…」 は filter も落とさない (2026-08-13 是正、 OP12-061)

OP12-061 ロシナンテ【起動メイン】「このターン中、次に自分が手札から登場させる**コスト4以上の
「トラファルガー・ロー」**の支払うコストは2少なくなる」。

**是正前の挙動**: overlay が **filter 無しの素の `reduce_play_cost: 2`** で、 2 重に違反していた。

1. 「トラファルガー・ロー」 以外の **どんなカードにも** -2 が効いていた
2. 対象外のカード (= 公式 Q&A が例に挙げる **コスト3の「ロー」**) が割引を食い潰していた

**是正**: `reduce_play_cost_filtered_turn {amount: 2, filter: {name: "トラファルガー・ロー",
cost_ge: 4}}` へ。 同日実装した 「次に」 = 1 枚消費 (`_consume_filtered_turn_reduction`) と
組み合わさって公式どおりになる。

⭐ **教訓**: 「次に登場させる〈filter〉」 型は **filter と 1 枚消費の 2 つ** を落としやすい。
`reduce_play_cost` (filter 無し) を使ってよいのは 公式テキストにも修飾が無い時だけ。

## 公式どおりで **問題なかった** もの (2026-08-13 バッチ 5)

- **df21c16a0a46** OP06-009 シュライヤ: 「相手のリーダーと同じパワーになる」 で元々のパワーが
  5000 になり、 付与ドンの +1000 が **その上に** 乗って 6000。
- **df1ea6e70e96** ST02-008: レストにできるのは **コストエリア** のドンだけ。 キャラに付与された
  ドンは選べない。
- **ded46c2cc5b2** OP12-034: 「属性(斬)を持つカード**か**緑のイベント」 は OR。 緑以外の斬カードも可。
- **dece419bceea** OP15-075: 後段の KO が空振りでも 前段の +1000 は独立に成立する。

### 検証不能 (カード未収録 = 要データ更新)

- **df2821dfaa7c** OP15-064 (他 cardqa_op_15 の Q1257/Q1258/Q1259/Q1269 も同型): 設問が前提とする
  **「すべてのカード名と特徴と属性を持つ」 リーダーが `db/cards.json` に存在しない**
  (全 330 リーダーを走査して 0 件)。 当 DB より新しい弾のカード。
  ⚠ `scripts/check_official_updates.py` は faq / cardqa / banlist の **変更を検知済** なので、
  [[onepiece-tcg-refresh]] でデータを最新化してから再判定する。
...
## 発動コストの離脱は **人間 modal 経路でも** 置換を通る (2026-08-13 是正、 OP01-047 × OP05-100)

**一次情報** (cardqa_op_05):

> Q: 「OP01-047 トラファルガー・ロー」の「自分のキャラ1枚を持ち主の手札に戻すことができる:」で
>    このキャラ (OP05-100 エネル) を選び、代わりに自分のライフを1枚トラッシュに置いた場合、
>    コスト3以下のキャラを手札から登場できますか？
> A: **いいえ、できません。**

同日に是正した OP05-087 × キュロス (KO コスト) と同じ一般則 — **コストの離脱も普通の離脱なので
置換効果がかかり、 置換を使えばコストは未払いになってコロン以降は起きない**。

⚠ **人間経路に別の穴があった**: `self_chara_cost_pick` (= 犠牲にする自キャラを人間が選ぶ modal) の
解決ハンドラが **独自にカードを zone 移動しており**、 cost handler 側に入れた置換チェックを
まるごと迂回していた。 → こちらも `try_replace_ko` 経由にした。

さらに置換が **任意** (「代わりに〜できる」) の場合、 使うかどうかは *次の* modal で決まるので、
コストの成否はその場では確定しない。 → `state._cost_sacrifice_watch` に victim を控え、
**modal チェーンが解決した時点で盤面を見て** 判定し (victim が場に残っていれば置換成立 = 未払い)、
未払いなら `_continuation` (= コロン以降の残り do) を破棄する。

| 人間の選択 | 結果 |
|---|---|
| エネルを選び **置換を使う** | エネルは場に残る / ライフ-1 / **コスト3以下は登場しない** |
| エネルを選び **置換を使わない** | エネルが手札へ / ライフ不変 / 登場する |
| 別キャラを選ぶ | 従来どおり (バウンス → 登場) |

**恒久ガード**: `test_op01_047_cost_bounce_replaced_by_enel_makes_effect_fizzle`

## 【トリガー】は 「条件不成立」 でも **発動できる** (2026-08-13 是正、 OP03-033)
...
| | 発動宣言 | 結果 |
|---|---|---|
| **発動コストが払えない** | **できない** (公式 4-10 / cardqa_st_22) | カードは通常どおり手札へ |
| **「〜の場合」 の条件が不成立** | **できる** | 効果が何も起きず、 カードは**トラッシュ**へ |

これは activate_main で確立済の 「コロン後の条件は効果のみを gate。 任意コストは条件不成立でも
支払える」 (cardqa_eb_04 の一般則) と同じ構造。

**是正前の挙動**: `trigger_lifecard_trigger` の合法性判定 (`_trigger_effect_activatable`) が
**効果の条件も gate に混ぜており**、 条件不成立だと 「発動できない」 = カードが **手札に加わって**
いた (= 公式より得をしていた)。 2026-08-11 にコスト payability を足した時に、 元からあった条件
チェックを分離し損ねたのが原因。

**是正**: 合法性判定を `_trigger_effect_cost_payable` (= **条件を見ない**) に分離。
`_trigger_effect_activatable` (条件 + コスト) は **AI の判断用** (`should_fire_trigger`) として残す。
条件不成立で発動するのは 「カードだけ失う」 ので AI は従来どおり見送る
= **自動対戦の挙動は不変 (matrix 中立)**。 変わるのは **人間が選べる範囲** だけ。

**恒久ガード**: `test_trigger_can_be_declared_even_when_its_condition_fails`
(条件不成立で発動→トラッシュ / 発動しない→手札 / 条件成立→登場 / AI は見送る、 の 4 対照)

## 公式どおりで **問題なかった** もの (2026-08-13 バッチ 6)

- **de87991d6658** ST31-003: 付与ドン合計が 3 未満になった瞬間に【ブロッカー】と +3000 を失う
  (静的効果なので `_recompute_static` が毎回張り直す)。
- **de131ec36ec1** ST13-004: ライフ → デッキへ置いたカードは裏向き (engine のデッキは向きを持たない
  = 構造的に公式どおり)。
...
## 2026-08-14: reveal_top_play で登場させなかった公開札はデッキ一番上に戻す (fixed)

**一次情報**: OP01-060 ドンキホーテ・ドフラミンゴ / cardqa_op_01 (c46c5af6d7b9)
> Q: この【アタック時】効果で公開したカードを登場させなかった場合、そのカードは表向きのままですか？
> A: いいえ、裏向きの状態でデッキの**一番上**にもどします。

**一般則**: 「デッキの一番上を公開し、(条件)の場合、登場させてもよい」型の効果で、テキストに
**「(残りを)デッキの下に置く」の明示が無い**カードは、 登場させなかった/非マッチの公開札を
**デッキの一番上に戻す** (公開しただけで動かさない)。 「その後、残りをデッキの下に置く」と
明記するカード (OP06-119 / OP07-048 等) だけが底に置く。

**違反と是正**: `reveal_top_play` の `rest_remain` が、 テキストに「下に置く」が無い 2 種で
`bottom` になっていた → `top` に是正 (overlay のみ、 Python/Rust 共通ソース):
- OP01-060 (本体 + _p1 + _p2 = 3 エントリ) — 「レストで登場させてもよい。」で終わり、 下に置く文言なし
- OP12-058 モンキー・D・ルフィ — 「登場させてもよい。登場させた場合…速攻。」で下に置く文言なし

Rust の `reveal_top_play` は `rest_remain=="top"` を `me.deck.insert(0, revealed)` で処理済
(effects.rs:7288) = overlay 変更のみで両エンジン一致を維持。

**回帰**: `tests/test_effect_interactions.py`
- `test_reveal_top_play_non_played_card_returns_to_top` — 非マッチ札が top に残る (対照)
- `test_reveal_top_play_rest_remain_matches_text_placement` — overlay 全走査ガード
  (テキストの配置指示「下に置く」=bottom /「上か下」=top_or_bottom / 明示なし=top と一致)

## 2026-08-14: 問題なかった確認 (conform、 再調査回避のため記録)

- **OP03-119** (cardqa_op_01): 「自ライフが相手より少ない場合」= 現在のライフ枚数。
  `self_life_lt_opp` = `len(me.life) < len(opp.life)` (現在値、 effects.py:2214)。
- **OP05-075** (cardqa_op_05): 相手のアタック時に新登場した同名キャラの【相手のアタック時】は
  同一アタック中に再発火しない (opp_attack はアタック宣言時に走査発火)。
- **OP05-047** (cardqa_op_05): 【ブロック時】の手札3枚以下条件はブロック宣言時に評価。
  カウンターステップで手札が減っても遡らない。
- **OP13-091/OP13-084** (cardqa_op_14): 元々パワーが `set_base_power(original:true)` で7000に
  書換わり、 元々パワー基準判定に反映 (`truly_original_power`、 core.py:523、 公式4-9-2-1)。
- **OP15-119** (cardqa_op_15): 相手イベント発動時、 イベント解決後に反応効果を発動。
- **OP10-003** (cardqa_op_10): 相手ターン中【ターン1回】自イベント発動時のドン追加は、
  イベント内の任意コストを払わなくても発動する (発動そのもので誘発)。
- **OP08-060** (cardqa_op_08): 登場時に相手ドン5以上で得た【速攻】は、 後で相手ドンが4以下に
  なっても消えない (give_keyword duration:turn は付与時点で確定)。
- **OP16-033** (cardqa_op_16): アクティブの本人がKOされる時、 代替コスト(自カード2枚レスト)に
  本人自身を含められる。
- **OP15-052** (cardqa_op_15): 元々パワー7000以下2枚が相手効果で同時離脱時、 自キャラ1枚を
  デッキ下に置く代替1回で2枚とも残す (同時離脱バッチ = 同一holderの代替コストは1回)。
- **ST12-003** (cardqa_st_12): 登場で自キャラが3枚になる場合、【登場時】(自キャラ2枚以下)発動不可
  (`self_field_count_le` は登場後の枚数 = 発動元自身を含む)。
- **OP15-041** (cardqa_op_15): 【起動メイン】コストで本人自身をデッキ下に置ける。 その後の
  【速攻】付与は不在対象で no-op。
- **OP09-098** (cardqa_op_09): 嵐脚でコスト-3された相手キャラを現在コスト4以下として KO 可
  (`opp_just_negated_cost_le_4` = `ip.base_cost` = 修正後の現在コスト)。
- **OP16-107** (cardqa_op_16): 手札0枚では【トリガー】(手札1枚捨てて登場)を発動不可
  (optional_cost_then payability が discard_hand を len(hand)<1 で払えないと判定)。
- **EB02-015/ST24-005** (cardqa_eb_02/st_24): 登場時「その後このターン終了時ドン1枚アクティブ」は
  他の【自分のターン終了時】を全て発動解決した後に発火 (schedule_at_self_turn_end)。
- **PRB02-014** (cardqa_prb_02): トラッシュ14でEB01-042起動時、 コスト(trash_self)でサボが
  トラッシュ入り15になりサボ手札コスト6-3=3、 スカーレット効果(cost3以下)で登場可
  (`play_from_hand` が in_hand_cost_minus 条件を効果時点で動的評価)。
- **OP06-028** (cardqa_op_06): 【アタック時】でドンをアクティブにしなくても(「〜まで」=任意)、
...
台帳が pending 0 に到達した後、 残っていた escalated 13 件を 1 件ずつ潰す。

## 遅延デッキアウト敗北は **0 枚到達で確定** する (OP15-022)

**一次情報** (cardqa_op_15、 2 問とも 「はい、敗北します」):

> Q1: デッキが0枚になったターンに、他の効果でデッキが1枚以上になった場合、
>     そのターン終了時に敗北しますか？
> Q2: デッキが1枚から0枚になり、その後 他の効果でデッキが1枚以上になり、さらに相手が
>     「OP09-097 闇水」でこのリーダーの効果を無効にした場合、このターンの終了時に敗北しますか？

**是正前の挙動**: END フェイズの判定が `not p.deck` (= **その時点で** 0 枚か) だったため、
**デッキが補充されると敗北を免れていた**。 ⚠ 2026-08-13 に別 Q&A を 「engine のデッキ0敗北判定は
ターン終了時チェック=一致」 として **conform と誤判定していた** のもこれが原因。

**是正**: `Player.deck_hit_zero_this_turn` を新設。 `_check_rule_defeat` が 0 枚到達時に立て、
END フェイズはこのフラグで判定して判定後にクリアする (ターンごと)。

⭐ **教訓**: 「ターン終了時にチェックしている」 ことと 「ターン中に起きた**事実**を記憶している」 ことは
別物。 コードの呼び出し位置だけを見て conform にせず、 **反例シナリオを実測する**。

| シナリオ | 結果 |
|---|---|
| 0枚のまま | ターン終了時に敗北 |
| 0枚 → 補充 | **ターン終了時に敗北** (公式) |
| 0枚 → 補充 → リーダー無効化 | **ターン終了時に敗北** (公式) |
| 0枚のまま → リーダー無効化 | 置換が消えて **その時点で** 即敗北 |

## 置換効果は **ターン終了時の離脱** にもかかる (EB04-044)

**一次情報** (cardqa_eb_04): 「OP11-092 ヘルメッポ」 の【登場時】で登場した EB04-044 コビーが
そのターン終了時に場を離れる時、 コビーの【ターン1回】置換 (手札1枚を捨てて場に残る) を
使えるか → **はい、できます**。

**是正前**: 一時登場キャラの回収 (`return_to_deck_bottom_at_turn_end`) と自己犠牲
(`trash_at_self_turn_end`) が **置換機構を通さず** 問答無用で場から除いていた。
→ 両経路を `try_replace_ko` (leave_kind=`return_to_deck_bottom` / `trash`) 経由に。

## 「自分のカード1枚をレストにできる」 の 「カード」 は **ドン‼ を含む** (OP14-020)

**一次情報** (cardqa_op_14): 「自分の場にある、**リーダー、キャラ、ステージ、ドン!!のうち1枚**を
アクティブからレストにすることで発動します」。

**是正前**: `rest_own_card` の pool が leader+characters+stages のみで **ドンを選べず**、
場のカードが全てレストだと発動できなかった。 → payability 2 箇所 + 支払い 2 箇所
(activate_main / optional_cost_then) でドンを頭数に入れた。 ⚠ filter 付きは除外
(ドンは特徴/コストを持たない)。

⭐ **AI 順を 非リーダー (power 昇順) → ドン‼ → リーダー に変更**。 リーダーをレストにすると
そのターンのアタックを失うので、 ドン 1 枚より高くつく。

## 公式どおりで **問題なかった** もの

- **b90bc64e1eb8** OP09-081 × PRB01-001: 【登場時】が **無効化されていても** 「【登場時】効果を
  持たないキャラ」 には該当しない (= サンジの起動メインの対象にできない)。 engine の target spec は
  **overlay (カードの素性)** で判定し runtime の無効化状態を見ないので公式どおり。
  ⭐ 「効果が無効になっている」 と 「効果を持たない」 は別物。

### 検証不能 (カード未掲載)

- **c069133c5e38** OP15-071: 「すべてのカード名と特徴と属性を持つ」 リーダー前提。 公式カードリスト
  未掲載 (生 HTML を grep して確認済) → n/a。 同型 5 件と同じ扱い。

## トリガーは **バッチ解決** — 誘発した効果は現バッチを追い越さない (OP07-019)

**一次情報** (cardqa_op_07、 OP07-019 ボニー):

> Q: 相手の「OP01-060 ドフラミンゴ」がアタックし、その【アタック時】効果で「OP07-045 ジンベエ」を
>    登場させました。この場合、この【相手のアタック時】効果で、相手のこの「ジンベエ」の【登場時】
>    効果で登場したキャラをレストすることはできますか？
> A: **いいえ、できません。** まず「ドフラミンゴ」の【アタック時】効果で「ジンベエ」が登場し、
>    **次にこのカードの【相手のアタック時】効果を発動します。その後**、「ジンベエ」の【登場時】効果で
>    手札からキャラが登場します。

**是正前の挙動**: 順序が **① → ③ → ②** で逆だった。 根因は 2 つ:

1. `apply_action` の攻撃宣言で `trigger_on_attack` が **末尾で drain していた** ため、
   攻撃側の【アタック時】が誘発した【登場時】まで解決し切ってから防御側を積んでいた。
2. `_pop_next_event` が **常にターンプレイヤー優先** だったため、 解決中に新たに誘発した
   ターンプレイヤーの効果が **既に積まれている非ターンプレイヤーの効果を追い越して** いた。

**是正**:

- 攻撃宣言では `on_attack` / `opp_attack` / `opp_attack_on_leader(_on_chara)` を
  **3 つとも先に enqueue してから 1 回だけドレイン** する (公式 7-1-1-3 「同時に発動」)。
- `resolve_triggers` を **バッチ解決** に変更。 ドレイン開始時点でキューにある集合を 1 バッチとし、
  その中はターンプレイヤー優先で解決。 **解決中に生まれた誘発は次バッチ** で、 現バッチを追い越さない。

⭐ **一般則**: 「同時に発動した効果」 はターンプレイヤーが先。 だが **後から誘発した効果は
「同時」 ではない** ので、 既に待っている相手の効果より後になる。

Rust は Vec の **先頭 `batch_len` 件** でバッチを表現する (新規 enqueue は必ず末尾に積まれるので等価)。

⚠ 影響範囲は広い (攻撃宣言まわり全般) が、 **pytest 全件 green のまま** = 既存の期待値と矛盾しない。

## 「ドン‼が付与された時」 は **1 枚につき 1 回** (OP02-002)

**一次情報** (cardqa_op_02): 「ST01-011 ブルック」 の【登場時】で自分のレストのドン!!**2枚**を
このリーダーに付与した時、 相手のキャラのコストを **-2** できますか → **はい、できます**。

**是正前**: 2 つ欠落していた。

1. **効果由来のドン付与** (`attach_don` / `attach_rested_don` / `attach_active_don`) が
   トリガーを **一切発火しなかった** (= ブルックで付与しても -0)。
2. アクション経由でも **付与アクションごとに 1 回** (= 2 枚まとめて付与しても -1)。

**是正**: `trigger_on_self_don_attached(count=N)` を新設し、 3 つの attach primitive と
2 つの action 経路から **付与枚数分** 発火する。 併せて **inline `execute_effect` → enqueue +
`_maybe_resolve`** に統一した (旧実装は対象を取る効果の人間 modal が 2 回目以降潰れた)。

⚠ `to_opp` / `owner_of_target` の付与は **付与された側** が 「自分」 なので owner 別に発火する。

## 同時 KO は **1 事象** — 3 相で解く (OP05-032 / OP03-090)

**一次情報**:

> (a) cardqa_op_05 / OP05-032 ピーカ: 「このキャラと同時にKOされた自分の3コスト以上のキャラを
>     レストにし、このキャラがKOされないことはできますか？」 → **はい、できます。**
> (b) cardqa_op_03 / OP03-090 ブルーノ: 「このカードと、コスト4以下の『CP』を含む特徴を持つ
>     自分のキャラが同時にKOされた場合、この【KO時】効果でこのカードと同時にKOされたカードを
>     登場させることはできますか？」 → **はい、できます。**

**是正前の挙動**: victim ごとに 「置換 → trash → 【KO時】」 を **逐次** 実行していた。 その結果

- (a) ピーカの置換が解決する時、 co-victim は **既に場を離れていて** レストの対象にできない
- (b) ブルーノの【KO時】が解決する時、 co-victim は **まだトラッシュに入っておらず** 選べない

**是正**: `ko_all_others` を **3 相** に。

| 相 | 内容 |
|---|---|
| **A** | 免疫 / 置換の判定 — **まだ誰も場を離れていない盤面** で行う |
| **B** | 生き残らなかった victim を **まとめて** トラッシュへ |
| **C** | 全員がトラッシュに入ってから【KO時】/【自分のキャラがKOされた時】/【相手の〜】を発火 |

⭐ **一般則**: 「同時に」 と書かれた除去は、 **判定 → 移動 → 誘発** を相ごとに揃える。
逐次処理すると 「先に消えた札を見られない / まだ来ていない札を選べない」 の 2 方向に破れる。

Rust も 1:1 でミラー (判定相は `peek_tagged` = 非消費、 除去相で `find_tagged` に切替)。

## 「属性X とバトルする時、このターン中 +N」 は **バトルごとに累積** (ST05-010)
...
**是正前**: `set_battle_pump_vs_attribute` を **power 計算時に毎回 +N する 「バトル中だけの補正」**
として実装していた。 同ターンの別バトルに残らず、 **累積もしなかった**。

**是正**: `game._apply_battle_attr_pump` を新設し、 **バトル開始時に `turn_buff` へ積む**
(そのバトルの power にも反映する)。 `_battle_attr_bonus` は金額算出のみに降格。

⭐ **読み分け**: 「このバトル中」 = battle_buff / 「**このターン中**」 = turn_buff。
テキストがどちらかを必ず確認する。 「〜する時、 このターン中」 は **発動のたびに積む**。
...
## 一般FAQ (よくある質問タブ) を台帳に取り込んだ (2026-08-21)

`db/faq_qa_status.json` のソースは長らく `db/faq/cardqa_*.json` **だけ** で、同じ `db/faq/` に
ある **一般FAQ 4 カテゴリ** (`base` / `detail` / `keyword` / `keyword_effect`) が一度も
conformance を通っていなかった。 台帳を 1,205 → **1,267 件** に拡張 (実ユニーク 70、
うち 8 件は cardqa と文面が同一なので **新規 62 件**。 各ファイルは公式ページの見出し重複で
2 重掲載されている)。

⚠ **拡張時の data-loss に注意**: `db/faq/*.json` は Bandai 著作物ゆえ全て gitignore で、
一次ソースが無い環境 (fresh clone / 公式サイト遮断のクラウドルーティン) が実在する。
`sync` の prune を **source 単位で gate** しないと、 読めなかった側の処理済 status ごと
消える。 cardqa には committed snapshot (`db/cardqa_tagged.json`) があるが一般FAQ には無い。

⭐ **一般則は cardqa より当たりが太い** — カード個別の解釈でなく **全試合に効く** ため。
実際 62 件から実バグ 3 件が出た (下記)。

### ① 「『X』を含む特徴」 を完全一致で実装していた

**一次情報** (`db/faq/base.json`):
> Q: 「『○○』を含む特徴を持つ」 とは、 《元○○》 や 《○○傘下》 などの特徴を持つ場合も
>    含まれますか？
> A: **はい、含まれます。**

engine には `feature` (完全一致) と `feature_contains` (部分一致) が **両方あった** のに、
overlay が書き分けを潰していた: filter 33 カード / 36 箇所 + リーダー条件 46 カード / 47 箇所。

実害: 『白ひげ海賊団』 → **元白ひげ海賊団 12 枚 + 白ひげ海賊団傘下 23 枚** が対象外 /
『B・W』 → **元B・W 39 枚** が対象外。

⚠ **rename だけでは黙って不発になる**: `power_pump` (target の兄弟キー) と `feature_or_name`
は `feature_contains` を読まなかった。 前者は Python/Rust 両方に追加、 後者は
**OP08-053 の白ひげサーチが 1 枚も引けなくなる** 形で backfill テストが検出した
([[feedback_verify_overlay_key_handled]] の実例)。

恒久監査 = `scripts/audit_feature_contains.py`。 **逆向き (部分一致の使いすぎ) も撃つ** —
特徴には `'月' ⊂ '光月家'` のような **偶然の** 部分文字列関係があり、 素の 「特徴《月》」 に
contains を使うと 光月家 まで拾う。 校正: 修正前 overlay で **83 件検出 → 修正後 0 件**。

### ② features/color を **全角 ／** で分割していなかった (副産物)

`cards.json` の `features` は `/` 区切りだが、 公式表記に **全角 ／ (U+FF0F)** のカードが
2 枚ある (OP06-057 「ドレスローザ／麦わらの一味」 / OP05-059 「四皇／百獣海賊団」)。
半角のみで split していたため **結合した 1 つの特徴** になり、 素の
`feature: "麦わらの一味"` filter に一致しなかった。 同じ組を半角区切りで持つカードが
32 枚 / 38 枚あるので分割が正。 → `engine/core.py` の CardDef 構築で両方を区切りとして扱う。

### ③ 【メイン】を持たないイベントをメインフェイズに発動できた

**一次情報** (`db/faq/keyword.json`):
> Q: イベントカードの【カウンター】効果を自分のメインフェイズに発動する事はできますか？
> A: **いいえ、できません。** …相手がアタックしたバトル中のカウンターステップにのみ
>    発動する事ができます。

`legal_actions` が EVENT を **無条件に** `PlayEvent` 候補にしていた。 該当は
【カウンター】専用 38 枚 + 【カウンター】+【トリガー】 176 枚 = **214 枚**
(= ドンを払って何も起きない非合法手。 AI の資源を溶かす)。
⚠ コードのコメントには 「overlay に main 効果がある場合のみ発動候補に」 と書かれていたのに
**実装が無かった** — コメントを実装の証拠にしない。
→ `_EVENT_MAIN_WHENS = {"main", "event_main"}` で gate。 Rust も `event_playable_in_main`
で同期 (**legal_actions が食い違うと parity が壊れる**)。

### ④ 【トリガー】からの自己効果コピーが **発動コストを踏み倒していた** (タダ撃ち)

**一次情報** (`db/faq/keyword_effect.json`、 OP03-074 独楽結び):
> Q: 【トリガー】効果 「このカードの【メイン】効果を発動する。」 を発動する場合、【メイン】
>    効果の 「ドン!!-2：」 の発動コストを支払わずに、【メイン】効果を発動できますか？
> A: **いいえ、発動できません。**

`fire_self_effect` が コピー先の `cost` を **一切見ていなかった**。 実測でドン 0 のまま
相手キャラをデッキ下へ送れた。 該当 = OP03-073 (pay_don 1) / OP03-074 (pay_don 2) + パラレル。
→ `_can_pay_counter_cost` で払えなければ発動せず、 払えれば `_pay_counter_cost` で支払う。
Rust も `can_pay_counter_cost_full` / `try_pay_counter_cost` で同期。

⭐ **一般則**: 「**このカードの【XXX】効果を発動する**」 系のコピーは、 **コピー先の発動
コスト・条件・無効化 gate をすべて引き継ぐ**。 コピー経路だけ gate を素通りする型は
既に OP09-081 (【登場時】無効) で 1 度踏んでおり、 今回はコストで同じ穴が空いていた。

## 「アタック対象変更」 は **そのアタック限り** — 持ち越すと別のアタックが曲がる (2026-08-21)

**発見経路**: 選択列挙 ON の Python↔Rust 差分 (`scripts/rust_choice_diag.py`)。
Rust だけが対象変更しない局面が複数出たので追ったところ、 **Python 側が
「撃っていないアタック」 の矛先を勝手に変えていた** = Python のバグだった。
Rust は `pending_attack_redirect` を action 境界で必ず None にするので巻き込まれなかった。

**何が起きていたか**: `state.pending_attack_redirect` は 「【相手のアタック時】の効果が
**そのアタック**の対象を変える」 ための action 内 transient。 消費するのは
`game.py` の **AttackLeader 経路だけ**で、

- **キャラへのアタック** (AttackCharacter) では立っても読まれず捨てられない
- AttackLeader でも途中 return (アタック不能 / 攻撃者離脱 等) で消費されない

ため、 値が **次のアタックまで生き残り**、 その後の 「効果を撃っていないアタック」 が
勝手にキャラへ曲げられていた (実測: OP14-060 紫ドフラのターン1回効果が、 同ターンの
別アタック 2 回に効いていた)。

**公式**: OP14-060 は 「【相手のアタック時】…**そのアタックの**対象を、 このリーダーか
自分の特徴《ドンキホーテ海賊団》を持つキャラ1枚にする」。 対象変更は **宣言されたその
アタック** にのみ効く。 別のアタックに持ち越す解釈はどこにも無い。

**是正** (`engine/game.py`): **action 境界で必ず落とす** (= 消費側に頼らない)。
- `apply_action` の `finally` の **最後** で `pending_attack_redirect = None`
  (⚠ 先頭で消すと漏れる — その手前の `resolve_triggers` が opp_attack 効果を解決して
   **立て直す** ことがある。 実測でこれを踏んだ)
- `ResolveChoice` 分岐は try/finally を通らないので **そこでも落とす**
  (選択の解決中に立った対象変更が次のアタックへ漏れる)
- AttackCharacter でも立っていたら破棄 (ログ付き)

⚠ **残る gap (未実装)**: 「キャラへのアタック」 に対する対象変更そのもの。 公式上は
キャラ戦でも対象を差し替えられるはずだが、 engine は AttackLeader 経路でしか差し替えを
実装していない。 いまは 「誤って別アタックに効かせる」 より 「そのアタックでは不発」 の方が
害が小さいので破棄している。 キャラ戦での差し替えは別途要実装。

⭐ **教訓**: **action 内 transient は 「消費する側」 が 1 経路しか無いと必ず漏れる**。
`state_snapshot.py` の除外コメントには 「action 境界では必ず None に戻る (消費即クリア)」 と
書いてあったが、 **実際には戻っていなかった**。 コメントの不変条件は入口で強制する
(= 読む前に必ずクリアする) こと。 そして **差分エンジンは 「片方だけが持つ transient」 を
炙り出す検出器として働く**。

## 「アクティブの/レストの キャラ」 は **盤面の状態** — CardDef 委譲だと制限が消える (2026-08-22)

**一次情報** (EB01-028、 公式テキスト):
> 【カウンター】自分のリーダーが特徴《インペルダウン》を持つ場合、自分のリーダーかキャラ
> 1枚までを、このバトル中、パワー+2000。その後、**相手は自身のアクティブのキャラ1枚を**、
> 持ち主の手札に戻す。

**engine は 「アクティブの」 を落としていた**。 しかも **両エンジンが別々の間違い方** をした:

| | 挙動 | 帰結 |
|---|---|---|
| Python | `_matches_filter` が未知キーを **黙って無視** → 制限なし | **レスト済のアタッカー** を戻す = バトル中断 |
| Rust | `matches_filter` は未知キーを **不一致扱い** → 0 対象 | 1 枚も戻さない = アタック通過 |

実測局面ではこの差が **勝敗そのもの** を変えた (Rust だけ game_over / winner 確定)。

**原因**: `rested` / `active` は **InPlay の状態** なのに、 `_matches_filter_ip` /
`matches_filter_ip` が両方とも CardDef 版へ委譲していた。 CardDef はレスト状態を持たない。

**是正**: cost/power と同じく **入口 (`*_matches_filter_ip`) で判定して strip** する
(`rested: true` = レスト / `active: true` = アクティブ、 `active` が両方あれば後勝ち)。
overlay の該当は **27 エントリ / 27 カード**で、 全件が公式テキストに 「アクティブ」 か
「レスト」 の語を持つことを機械照合済 (食い違い 0)。

⭐ **教訓**: これは 「両エンジンが同じ間違いをする」 より質が悪い **「同じ overlay から
別々の間違いが出る」** 型。 差分検証では出るが、 **どちらも公式と違う** ので
「Rust を Python に合わせる」 と直らない。 CLAUDE.md の既知の罠
(「未知キー方針が Python と逆。 どちらが正かは公式テキストで決める」) の実例。

## 【相手のアタック時】の発動判断は 【アタック時】の do より **前** (2026-08-22)

**engine の一次情報** (公式ルールでなく解決モデル): Python は攻撃解決で
`state.resolving = True` に固定したまま ① `trigger_on_attack` ② `trigger_on_opp_attack`
を **両方 enqueue し切ってから** `resolve_triggers` で drain する。
【相手のアタック時】のコスト支払い・【ターン1回】マーク・AI の発動 EV 判定は
**enqueue の時点** = 【アタック時】の do がまだ走っていない盤面で行われる。

Rust は `fire_on_attack` を do まで実行してから `fire_opp_attack` を呼んでいたため、
**【アタック時】が誘発した【KO時】でライフが増えた後** の盤面で防御 EV を測っていた。
実例: OP07-019 緑ボニー (【相手のアタック時】【ターン1回】) が、 アタッカーの【アタック時】
KO → OP06-104 菊之丞【KO時】「デッキ上1枚をライフへ」 でライフが 1→2 になった結果、
「ライフ 1 以下なら守りに価値あり」 の判定を外して **Rust だけ発動を見送っていた**
(= 【ターン1回】マークも付かず、 同ターンに再発動しうる状態だった)。

**是正**: Rust の攻撃解決を Python と同じ **「判断 → 判断 → 解決」** 順に組み替えた。
`collect_on_attack` / `collect_opp_attack` (支払い・判断のみ) と
`fire_on_attack_collected` / `fire_opp_attack_collected` (do のみ) に分割し、
両方の収集を済ませてから do を流す。
⚠ 収集と発火の間に盤面が動くので、 発動元は **位置 index でなくトークン** で持ち回る。
...
## 「ドン!!−N」 は **付与ドンからも払う** — 判定と支払いの食い違いでタダ撃ちできた (2026-08-22)

公式の 「ドン!!−N」 は **場のドン** を N 枚ドン!!デッキに戻すコスト。 「場のドン」 には
コストエリアの アクティブ/レスト だけでなく **キャラ・リーダーに付与されているドン** も含む
(FAQ 準拠、 engine は `_pay_don_from_field` で実装済)。

ところが **起動メインの発動コストだけ** が area (active/rested) しか見ておらず、

- 払えるかの判定 (`_can_pay_activate_main` / Rust `pay_don_capacity`) は **付与ドンを数える**
- 実際の支払い (`_fire_activate_main_inner` / Rust `fire_activate_main`) は **数えない**

ため、 **付与ドンしか残っていない局面で 「払えると判定されて 1 枚も払わない」** = タダ撃ちに
なっていた。 カウンター/登場時などの when-effect 側は元から `_pay_don_from_field` 経由で正しい。

実害は 2 つ:
1. **ルール違反** (コストを払わずに効果を得る)。
2. **AI の空転**: コスト 0 で何度でも撃てるので、 greedy が同じ起動メインを選び続ける
   (「ターンを終える」 より評価が高いため)。 self-play が **200,000 step 上限** に達する
   game が実在した (OP15-060 エネル、 40 game 中 2)。 列挙 ON/OFF どちらでも発生。

是正: 両エンジンとも `_pay_don_from_field` / `pay_don_field` (area → 付与ドン) に委譲。
⚠ **AI vs AI の挙動が変わる**ので matrix は stale になる (該当局面は稀)。

## 「ステージ1枚まで」 が **両エンジンとも silent no-op** だった (OP15-054、 2026-08-23)

OP15-054 の【メイン】選択肢②:
「**ステージ1枚までを、 持ち主の手札に戻す。**」

overlay は `{"return_to_hand": {"type": "any_stage_n_1"}}` と正しく書けていたが、
**`any_stage_n_N` を解決する実装がどちらのエンジンにも無かった**:

- Python `_resolve_target` は未知 spec を **`[]` で返す** = 対象ゼロ = **何も起きない**
- Rust `resolve_target` は未知 spec を **None (= 明示 bail)** で返す

つまり Python では 「選択肢②を選んでも盤面が動かない」、 Rust では bail。
差分検証は **両方おかしい (silent no-op と bail)** ので MISMATCH を出さず、
**選択列挙で選択肢②が選べるようになって初めて露見した** ([[project_choice_search_in_engine]])。

是正: 両エンジンに `any_stage_n_N` を実装。 「相手の」 の修飾が **無い** ので
**両陣営のステージ** が対象 (= 「キャラ1枚まで」 と同じ一般則、 本ファイル冒頭の書き分け参照)。
AI 順は相手のステージ優先 (妨害価値が高い)、 人間/探索は target_pick で選ぶ。

⭐ **教訓**: 「AI が選ばない選択肢」 は **実装が無くても誰も気づかない**。 選択を探索に載せると
そこが初めて踏まれる = **選択列挙は未実装カードの検出器にもなる**。
