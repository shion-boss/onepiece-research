...
| **R2** | ルール port (`game.py`: legal_actions/apply_action/turn 進行/戦闘/ライフ)。 action を canonical エンコード (card_id+zone位置、 iid 非依存) して両 engine で replay、 全 step digest 一致 | 🔄 着手: **AttachDon 128/128 + EndPhase/phase機械 160/160 一致 (2026-07-29)**。 phase 機械 (untap/draw/don refresh/_reset_turn_buff/ownership) 移植済。 残 = PlayCharacter/Event, Attack/戦闘, ActivateMain。 ⚠ これらは効果 (on_play/on_attack/activate) + 戦闘を伴うので **R3 (effects) が critical path** |
