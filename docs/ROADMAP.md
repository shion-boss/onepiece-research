...
工数は 2 軸で記載: 「人間ペース」 と 「Claude (= AI 開発者) ペース」。
Claude ペースは僕が継続的に作業した場合の wall-clock (= 計算時間含む) を示す。

| Phase | 内容 | 状態 | 期待効果 | 人間ペース | Claude ペース |
|---|---|---|---|---|---|
| 1 | カード DB | ✅ 完了 | — | — | — |
| 2 | ルールエンジン + 効果 DSL | ✅ 完了 | — | — | — |
| 2.5 | カード効果オーバーレイ (= 全 4,518 枚) | ✅ 完了 | — | — | — |
| 3 | 基本 AI (Greedy / MCTS / Lookahead) | ✅ 完了 | — | — | — |
| 4 | メタデッキ DB | ✅ 完了 | — | — | — |
| 4.5 | PlanningAI (= ターン beam search) | ✅ 完了 | cross +26pt | — | — |
| 5 | デッキビルダー | ✅ 完了 | — | — | — |
| 6 | Next.js UI | ✅ 完了 | — | — | — |
| **7** | **AI ヒューリスティック層強化** | ✅ 完了 (2026-05-14) | +15〜30pt | **2-3 週間** | **1 日** (= 7A〜7K) |
| **8** | **学習基盤 (= self-play AI)** | 📋 計画 | +20〜40pt | **2-4 ヶ月** | **1-2 ヶ月** (計算 bound) |
| **9** | **分散コンピューティング** | 📋 計画 | コミュニティ拡大 | **1-3 ヶ月** | **3-4 週間** |
| 10+ | 超人 AI / 汎用 / ナッシュ均衡 | 長期 | — | 6-12 ヶ月 | 数ヶ月 (計算 bound) |

Phase 7 が現状の直近フェーズ。 8 と 9 は並走可能 (= 8 で 学習基盤完成 → 9 で配布)。

### Claude ペースの内訳

「Claude ペース」 で速くなる / ならない要因:

**速くなる**:
- ロジック実装 + テスト (= 既存パターン流用で 数時間/sub-task)
- リファクタ + ドキュメント同時並行
- マルチタスク並走 (= 7A〜7F を交互に進める等)

**速くならない**:
- **計算時間**: 1M self-play は CPU 速度で決まる、 僕も待つだけ
- **NN 学習**: GPU 必要、 学習時間 そのまま
- **ユーザー応答の往復**: 仕様確認 / レビュー / 方針判断は人間時間
- **マシン環境 / OS 依存**: WASM 化 / 配布パッケージ等

実用判断:
- Phase 7 は **今週中に完成可能** (= 平日数日 + matrix 検証時間)
- Phase 8 は **計算リソース次第** (= 1 CPU で 1-2 ヶ月、 並列で短縮)
- Phase 9 は WASM 抜きなら 1 週間、 WASM 込みで 3-4 週間
