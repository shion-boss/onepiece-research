...
# ⚠ 2026-08-24 是正: 10 件が `"OP09-XXX"` 等の **プレースホルダのまま** で、 該当リーダーが
#   one-hot に一生載らなかった (= 常に unknown 扱い)。 さらに実在した 6 件のうち
#   `OP14-079` は 「緑紫ドフラ (1342)」 とコメントされていたが実際は **黒クロコダイル (1385)** で、
#   1342 は `OP14-060`。 = ラベルも間違っていた。
#   → `decks/cardrush_*.json` の `leader_id` から実データで再構成した (slug 昇順の初出順)。
# ⚠ one-hot の並びが変わるので、 **旧エンコーディングで学習した重みとは非互換**。
#   使うのは nn_eval / weight_nn (= Plan D/F の撤退済み路線) だけで、 配備 value
#   (agnostic GBM) は state_encoder を使わないため配備影響は無い。
KNOWN_LEADER_IDS = [
    "OP14-060",    # 紫ドフラミンゴ (1342)
    "OP14-079",    # 黒クロコダイル (1385)
    "OP13-079",    # 黒イム (1392)
    "OP15-002",    # 赤青ルーシー (1399)
    "OP11-041",    # 青黄ナミ (1439)
    "OP14-020",    # 緑ミホーク (1453)
    "OP15-058",    # 紫エネル (1454)
    "OP15-098",    # 空島ルフィ (1455)
    "OP13-002",    # 赤青エース (1456)
    "OP09-062",    # 紫黄ロビン (1483)
    "OP14-041",    # 青黄ハンコック (1492)
    "OP13-001",    # 赤緑ルフィ (1525)
    "OP08-098",    # 黄カルガラ (1549)
    "OP16-080",    # 黒黄ティーチ (1594)
    "OP11-040",    # 青紫ルフィ (1608)
    "OP12-061",    # 紫黄ロシナンテ (1609)
]
