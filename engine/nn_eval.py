...
def get_model() -> Optional[nn.Module]:
    ...
    train_nn_eval.py が保存した SimpleEvaluator (= 78 dim) を 優先 load。
    """
    ...
    try:
        # SimpleEvaluator (= 78 dim) 優先 (= train_nn_eval.py 出力形式)
        if feature_keys is not None:
            model = SimpleEvaluator(input_dim=len(feature_keys))
        else:
            model = NNEvaluator(input_dim=encoded_dim())
...
def compute_score_nn(state: GameState, me_idx: int) -> Optional[float]:
    ...
    SimpleEvaluator (= 78 dim features) の場合は compute_breakdown 経由で features 取得。
    NNEvaluator (= 172 dim) の場合は state_encoder 経由。
    """
    model = get_model()
    if model is None:
        return None
    try:
        feature_keys = get_feature_keys()
        with torch.no_grad():
            if isinstance(model, SimpleEvaluator) and feature_keys is not None:
                # SimpleEvaluator: compute_breakdown の diff 値を 78 dim features に
                from .eval import compute_breakdown
                bd = compute_breakdown(state, me_idx)
                features = [float(bd.get(k, {}).get("diff", 0)) for k in feature_keys]
            else:
                features = encode_state(state, me_idx)
