...
> ⚠ **「公式準拠 100%」 は 2026-08-09 時点で 達成していない (= 目標であって現状ではない)**。
> 公式 Q&A 全 1,205 件の conformance ([[project_faq_conformance_routine]]) で **検査済 416 件
> (n/a 49 除く) のうち 71 件 = 約 17% が違反 (fixed)**、 conform 345 件。
> 壊れていたのは主に **カード個別の効果解釈** (コスト gate 欠落 = タダ撃ち / 対象範囲を片側限定 /
> 印刷値と現在値の取り違え) で、 中核ルール (ターン進行・DON・ライフ・KO・攻防解決) は概ね正しい。
> 中核の例外はバトル中断 (2026-08-04 是正) と **発動コスト由来トリガーの解決順** (2026-08-09 是正、
> 公式 8-4-1-3〜5 / cardqa_op_14) の 2 件。
> **escalated (= 要深掘りで保留) は 0 件**。 **未処理 740 件 = 全体の 61.4% は未検査**。
