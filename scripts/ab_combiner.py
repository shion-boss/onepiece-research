...
side A の手番だけ ONEPIECE_EVAL_COMBINER を立てて beam を combiner value で走らせ、 side B は
配備の生 value。 combiner が「人間に近い / より強い」手を選べているかの直接検証。

⚠ first-player 優位を避けるため a_idx(A がどちら側か)と fp(先攻)を独立に振る。 N≥80 推奨。

  AB_SLUG=cardrush_1454 AB_COMBINER=db/_selfplay/eval_combiner29.pkl AB_N=80 AB_WORKERS=8 \
...
COMBINER = os.environ["AB_COMBINER"]  # combiner pkl path
...
def _one(seed):
    ...
    while not state.game_over and state.turn_number < 50 and n < 1500:
        ...
        if cur == a_idx:
            os.environ["ONEPIECE_EVAL_COMBINER"] = COMBINER
        else:
            os.environ.pop("ONEPIECE_EVAL_COMBINER", None)
        try:
...
def main():
    ...
    wr = a / max(1, a + b) * 100
    print(f"{SLUG}  combiner={Path(COMBINER).name}  A(combiner) {a} - {b} B(raw value)  "
